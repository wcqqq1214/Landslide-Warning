...
**当前状态：v2.1 概率 PINN 方案已完成，尚未实现或训练；保留 B+ 基线。**
方法仍只限定 ConvLSTM/PINN；v2.0 候选未达标，已停止。
...
| [v2.1 待实施方案](docs/ootang_probability_pinn_plan.v2.1.md) | 受限力偏差概率 PINN、首模型退出条件与总投入上限 |
...
v2.1 已固定具体候选、完整判据、总时间上限和失败退出条件；后续按用户具体请求执行。
方案完成不代表训练已启动或预测有效性已验证。
