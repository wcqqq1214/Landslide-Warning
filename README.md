...
> 当前代码已跑通完整研究框架；现有结果属于 Figshare 发布物化日序列上的探索性内部时间验证，不应表述为最终确认性泛化证据。该序列具有强自然月分段三次指纹，原始观测锚点和日值生成方法尚未提供，当前 `data_gate=blocked`。
...
| 数据血缘 | 已核验 Figshare 原文件、月内三次指纹和模型边界；原始锚点/生成算法/MJ-ATU 映射未解决，正式结果阻断 |
...
- 当前输入不能称为已验证的独立原始逐日 GNSS；正式预测与预警还必须先恢复原始锚点、日值处理链和点位映射，并在每个时间折内重新生成日序列。
