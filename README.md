...
**当前状态：按导师补充要求，先定位困难尾段并复核已有预测；暂缓 v2.1 PINN，保留 B+。**
导师允许图示尾段存在偏差；原图已定位为 ATU1/MJ3 的 2019–2020 年预测窗，精确尾段截点仍待明确。
已整理[一次 6pro 定向补审材料](docs/ootang_web_route_review_prompt.md)，当前没有分段效果或补审结论。
...
| [v2.2 当前方向](docs/ootang_tail_scope_and_direction.v2.2.md) | 导师尾段补充要求、一次分段复核与后续方向选择 |
...
后续先按 v2.2 定位测点/日期，再复核同一范围的主要时段、尾段及完整窗口；不按模型成绩挑截点。
复核后决定先完善概率输出还是再推进 PINN；不覆盖旧判据、改判负结果或重置预算。
