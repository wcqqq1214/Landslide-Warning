...
| V0/切线角融合 | 已跑通 | 等速阶段仍需导师或专家确认后才能冻结 |
...
| `code/tangent_stage_review.py` | 生成关键测点等速阶段专家复核材料 |
...
| `figures/tangent_angle/review/MJ1_stage_review.png` | MJ1 等速阶段复核图 |
| `figures/tangent_angle/review/MJ3_stage_review.png` | MJ3 等速阶段复核图 |
| `figures/tangent_angle/review/MJ9_stage_review.png` | MJ9 等速阶段复核图 |
...
- V0 和切线角规则已跑通，但切线角等速阶段尚未人工批准，不能冻结确认性切线角结论。
...
5. `figures/shap/*.png`
6. `figures/tangent_angle/review/*.png`

不建议展示本地缓存和临时文件，例如 `__pycache__/`、`.venv/`、`tmp/`。`figures/` 中大量 CSV 是复现和审计材料，仓库中应保留，但汇报时通常只需要核心图和汇总表。
