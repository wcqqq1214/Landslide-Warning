...
from convlstm import capacity_sensitivity as capacity
...
class ConvLSTMCapacitySensitivityTests(unittest.TestCase):
    def test_frozen_protocol_fails_closed_before_capacity_rerun(self):
    ...
    def test_outputs_are_isolated_in_versioned_stage_bundle(self):
        expected = (
            ...
            / "fixed120_v1"
            / "capacity_sensitivity_v1"
        )

        self.assertEqual(capacity.OUT_CANDIDATES.parent, expected)
        self.assertEqual(capacity.OUT_METRICS.parent, expected)
        self.assertEqual(capacity.OUT_PREDICTIONS.parent, expected)
    ...
    def test_protocol_matrix_and_parameter_counts_are_predeclared(self):
        ...
        self.assertEqual(capacity.model_parameter_count(8), 4379)
        self.assertEqual(capacity.model_parameter_count(16), 13363)
