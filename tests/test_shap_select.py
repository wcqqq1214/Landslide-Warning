...
from explainability import shap_select  # noqa: E402
from warning import warning_thresholds  # noqa: E402
...
class ShapSelectTests(unittest.TestCase):
    def test_outputs_are_grouped_under_shap_figures(self):
        ...
        outputs = [
            ...
            shap_select.OUT_PROVENANCE_JSON,
        ]
    ...
    def test_compute_v0_uses_thesis_monthly_rate_formula(self):
        ...
        self.assertAlmostEqual(
            result["v0_orange_threshold"], 5 * result["v0_mm_per_month"]
        )
        self.assertAlmostEqual(
            result["v0_red_threshold"], 10 * result["v0_mm_per_month"]
        )
        self.assertEqual(
            result["v0_estimation_method"],
            "thesis_steady_stage_velocity_statistic",
        )
        self.assertEqual(
    ...
    def test_build_warning_frame_uses_station_specific_v0_and_max_level(self):
        ...
        self.assertEqual(
            warning_frame["MJ9_warning_level"].tolist(), [-1, -1, -1, 0, 1, 1]
        )
        self.assertEqual(
            warning_frame["MJ1_warning_level"].tolist(), [-1, -1, -1, 0, 1, 2]
        )
    ...
    def test_build_lagged_samples_uses_five_day_history_and_dynamic_v0_labels(self):
        ...
        self.assertIn("disp_delta_v_lag5", X.columns)
        self.assertNotIn("disp_accel_lag5", X.columns)
    ...
    def test_evenly_spaced_date_sample_keeps_all_stations(self):
        ...
        sample, selected = shap_select.evenly_spaced_date_sample(
            X,
            meta,
        ...
        self.assertEqual(selected.tolist(), [dates[0], dates[2], dates[4]])
        self.assertEqual(len(sample), 6)
        self.assertEqual(set(meta.loc[sample.index, "station"]), {"A", "B"})

    def test_importance_frame_retains_explicit_provenance(self):
        frame = shap_select.importance_frame(
            np.array([[1.0, 0.5], [3.0, -0.5]]),
            ["disp_delta_v_lag1", "Rainfall_lag1"],
            provenance={"explained_model": "NGBoost", "target_status": "exploratory"},
        )

        self.assertEqual(frame.iloc[0]["feature"], "disp_delta_v_lag1")
        self.assertEqual(set(frame["explained_model"]), {"NGBoost"})
        self.assertEqual(set(frame["target_status"]), {"exploratory"})
