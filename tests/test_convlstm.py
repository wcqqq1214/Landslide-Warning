...
class ConvLSTMForecastTests(unittest.TestCase):
    ...
    def test_model_input_scaling_ignores_post_fit_values(self):
        ...
        inputs, scale = convlstm.make_model_inputs(
            df,
            disp,
            2,
            interp,
    ...
    def test_elevation_grid_changes_when_station_elevation_changes(self):
        ...
        interp, _ = convlstm.make_interpolator(
            xy,
            convlstm.GRID_H,
            convlstm.GRID_W,
        )

        baseline = convlstm.make_elevation_grid(elevation, interp)
        changed_elevation = elevation.copy()
        changed_elevation[0] += 100.0
        changed = convlstm.make_elevation_grid(changed_elevation, interp)
