...
from explainability import shap_stability  # noqa: E402
...
class ShapStabilityTests(unittest.TestCase):
    def test_outputs_are_grouped_under_stability_directory(self):
        outputs = [
            ...
            shap_stability.OUT_STATION_FEATURE_IMPORTANCE,
            shap_stability.OUT_STATION_FEATURE_STABILITY,
    ...
    def test_legacy_acceleration_feature_is_rejected(self):
        with self.assertRaisesRegex(ValueError, "未注册特征组"):
            shap_stability.assign_feature_group("disp_accel_lag1")
    ...
    def test_station_feature_stability_keeps_station_stratum(self):
        frame = pd.DataFrame(
            {
                "task": ["regression", "regression"],
                "station": ["MJ9", "MJ9"],
                "fold": [1, 2],
                "feature": ["disp_delta_v_lag1"] * 2,
                "feature_group": ["displacement_kinematics"] * 2,
                "normalized_share": [0.3, 0.2],
                "rank": [1.0, 2.0],
                "direction_spearman": [0.8, 0.6],
            }
        )

        result = shap_stability.summarize_feature_stability(
            frame,
            strata=("station",),
        ).iloc[0]

        self.assertEqual(result["station"], "MJ9")
