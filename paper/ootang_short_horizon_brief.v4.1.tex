...
\takeaway{\textbf{在线回归＋短期误差反馈表现最好。}神经初态模型的预测整体收益仍未达标；\Bplus{} 物理参照的额外收益尚不稳定。}
...
\takeaway{\textbf{神经初态模型采用 \Bplus{} 严格递推，满足求解器数值容差；预测整体收益仍未达标。}}
...
\textbf{神经初态估计与 \Bplus{} 严格递推：}开发整体未改善；后期 1--7 天平均误差下降，3--7 天达到预定概率改善条件，但逐点均值条件未满足，仍落后在线回归。后期按完整比较补齐，未重选。\par
