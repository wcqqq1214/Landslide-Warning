...
def main(attempt, version):
    ...
    try:
        ...
        assert exports["source_code_sha256"] == o.sha(
            c.ROOT / "code/tide_features/figures.py"
        )
        trees = {
        ...
        for rec in records:
            ...
            if kind == "summary":
                ...
                x = np.arange(3)
                f = read(root / "analysis/factorial_effects.csv")
                part = f[
                    (f["seed"].astype(str) == "ensemble")
                    & (f["point"] == r["point"])
                    & (f["metric"] == "rmse")
                    & (f["factor"] == r["factor"])
                ].set_index("origin")
                yy = [part.loc[n, "effect"] for n in cfg["origins"][1:]]
            elif kind == "point":
                x = np.arange(4)
                yy = [
                    r.get("multiplier", 1)
        ...
        result = subprocess.run(
            [
                ...
                str(c.ROOT / "code/tide_features/figures.py"),
                "--json",
            ],
            capture_output=True,
            text=True,
