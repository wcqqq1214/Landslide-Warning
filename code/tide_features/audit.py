...
def main(attempt):
    ...
    try:
                ...
                for seed in cfg["seeds"]:
                    ...
                    for step in cfg["checkpoints"]:
                        ...
                        assert (
                            ...
                            == o.sha(
                                c.old.CONFIG if arm in cfg["reuse_arms"] else c.CONFIG
                            )
                        )
                        assert (
                            cs == sc
        ...
        diffs = read("factorial_effects")
        assert len(diffs) == 2520
        for r in diffs.to_dict("records"):
            ...
            d, kin, hyd, p = [values[a] for a in cfg["arms"]]
            expected = {
                "K_at_H0": kin - d,
                "K_at_H1": p - hyd,
                "H_at_K0": hyd - d,
                "H_at_K1": p - kin,
                "K_average": ((kin - d) + (p - hyd)) / 2,
                "H_average": ((hyd - d) + (p - kin)) / 2,
                "interaction": p - kin - hyd + d,
            }
            close(
        ...
        assert outcome["stable_new_arm_vs_full_physics"] == {
            a: all(
                p["mean_pass"] and p["seed_agreement_pass"]
                for p in pairs
                if p["candidate"] == a and p["reference"] == "TiDE_PHYS"
            )
            for a in cfg["new_arms"]
        }
        receipt.update(
            status="passed",
            checkpoints=models,
