"""Five mentor-style figures for all four new arms and the fixed comparison."""
...
NAMES = {
    ...
    "TRANSFORMER_BRES_COND": "Transformer 残差",
...
def main(attempt):
    ...
    fig, axes = canvas(
        "四点完整 293 日预测：两种新结构与既有方法比较",
        "九种固定方法，同一完整日期｜均值对比；四个新模型区间见配套图",
    )
    ...
    fig.text(
        0.5,
        0.044,
        "神经模型主线均为三个种子均值；旧 TCN 和基线复用同协议结果，未重新训练。",
        ha="center",
        fontsize=7.6,
    )
