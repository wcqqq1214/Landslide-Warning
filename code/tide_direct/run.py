...
def train():
    ...
    for n, end in zip(cfg["origins"], cfg["ends"]):
        ...
        means = {k: pm[k] for k in cfg["controls"]}
        seeds = {k: ps[k] for k in cfg["controls"]}
        for arm in cfg["arms"]:
            dataset = c.arrays(
...
def score():
    ...
    for n, end in list(zip(cfg["origins"], cfg["ends"]))[1:]:
        ...
        comparisons = [(a, b) for a in cfg["arms"] for b in cfg["controls"]] + [
            ("TiDE_PHYS", "TiDE_DATA")
        ]
        ...
        for ident, group in [("ensemble", means)] + [
            ...
            va = o.scores(y[n:end], group["TiDE_PHYS"], sigmas["TiDE_PHYS"])
            vb = o.scores(y[n:end], group["TiDE_DATA"], sigmas["TiDE_DATA"])
            for point, a, b in [("average", o.summarize(va), o.summarize(vb))] + [
                (cfg["points"][p], va[p], vb[p]) for p in range(4)
            ]:
                for metric in (
                    "mae",
                    "rmse",
                    ...
                    differences.append(
                        dict(
                            ...
                            relative_change=(a[metric] / b[metric] - 1)
                            if b[metric]
                            else None,
                        )
                    )
    ...
    policy = [p for p in pairs if p["reference"] == "TiDE_DATA"]
