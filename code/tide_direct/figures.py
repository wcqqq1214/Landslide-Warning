...
def main(attempt):
    ...
    qa = Path(tempfile.mkdtemp(prefix="ootang-tide-qa-"))
    script = (
        Path.home() / ".codex/skills/nature-figure/scripts/audit_panel_alignment.py"
    )
    spec = importlib.util.spec_from_file_location("tide_alignment", script)
    ...
    name = "tide_comparison"
    fig, axes = canvas(
        "TiDE直接预测：物理输入的跨时段作用",
        "两版同结构、同样本、固定400次更新 | 每组3个种子 | 无位移反馈的293日条件预测",
    )
    ...
    for s in [0, 1, 2, None]:
        ...
        for n in cfg["origins"][1:]:
            a, b = (
                (summary.loc[(n, m), "rmse"] for m in cfg["arms"])
                if s is None
                else (ss.loc[(n, m, s), "rmse"] for m in cfg["arms"])
            )
            vals.append(b - a)
        ...
        line(
            ...
            x,
            vals,
            dict(type="paired", seed=s),
            color=COLORS["TiDE_PHYS"],
            lw=1.4,
    ...
    fig.text(
        0.5,
        0.09,
        "b负值表示加物理组误差更低；实线为集成预测差，浅色点为同种子差。d虚线为逐点80%工作门槛。",
        ha="center",
        fontsize=7.2,
    )
    fig.text(
    ...
    for m, name in [("TiDE_DATA", "data_final"), ("TiDE_PHYS", "phys_final")]:
    ...
    o.write_json(
        ...
        dict(
            ...
            source_code_sha256=o.sha(c.ROOT / "code/tide_direct/figures.py"),
            backend="python",
            width_mm=240,
            height_mm=170,
        ),
    ...
    print(
        {"figures": 3, "panels": 12, "records": len(records), "folder": str(out)},
        flush=True,
    )
