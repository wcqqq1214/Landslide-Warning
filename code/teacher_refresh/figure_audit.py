...
def main(attempt,version):
    cfg=c.spec();c.guard();root=c.ROOT/cfg['out'];folder=c.ROOT/cfg['figures']/version
    out=root/attempt;out.mkdir(exist_ok=False)
    o.verify_lock(root/'audit_lock.json');o.verify_lock(folder/'artifact_lock.json')
    ...
    try:
        ...
        seeds=read(root/'analysis/seed_summary.csv').set_index(['origin','method','seed'])
        points=read(root/'analysis/metrics_by_point.csv').set_index(['origin','method','point'])
        arrays=o.load_npz(folder/'source_arrays.npz');y=o.read_labels(c.ROOT/cfg['data'],1461)
        _,dates=o.read_forcing(c.ROOT/cfg['data'],1461);dx=mdates.date2num(pd.to_datetime(dates))
        close('observed',arrays['observed'],y,0);assert np.array_equal(arrays['dates'],dates)
        close('bplus',arrays['bplus_full'],c.bank(cfg)['cached'][1168]['mean'],0)
        ...
        assert exports['source_code_sha256']==o.sha(c.ROOT/'code/teacher_refresh/figures.py')
        trees={v['name']:ET.parse(folder/v['svg']).getroot() for v in exports['figures']}
        records=o.read_json(folder/'plot_records.json')
        for rec in records:
            r=rec['recipe'];key=rec['gid'];kind=r['type'];x=None;yy=None
            if kind=='summary':
                x=np.arange(3);yy=[summary.loc[(n,r['method']),r['metric']] for n in cfg['origins'][1:]]
            elif kind=='paired':
                s=r['seed'];x=np.arange(3)+(0 if s is None else (s-1)*.045)
                def value(n,m):return summary.loc[(n,m),'rmse'] if s is None else seeds.loc[(n,m,s),'rmse']
                yy=[value(n,'G_REFRESH')-value(n,'G_CACHED') for n in cfg['origins'][1:]]
            elif kind=='point':
                x=np.arange(4);yy=[r.get('multiplier',1)*points.loc[(1168,r['method'],p),r['metric']] for p in cfg['points']]
            elif kind in ('trajectory','band'):
                m,p=r['method'],r['point'];x=dx if m in ('observed','bplus_full') else dx[1168:]
                if kind=='trajectory':
                    data=arrays[m] if r['seed'] is None else arrays[m+'__seeds'][r['seed']]
                    yy=data[:,p]-y[0,p]
                else:
                    mu=arrays[m][:,p]-y[0,p];half=norm.ppf((1+r['level']/100)/2)*arrays[m+'__sigma'][p]
                    close(key+'/lower',rec['lo'],mu-half,0);close(key+'/upper',rec['hi'],mu+half,0)
                    v=np.asarray(rec['vertices']);assert len(v)==589
                    close(key+'/polygon_lower',v[1:294,1],mu-half,0)
                    close(key+'/polygon_upper',v[295:588,1],(mu+half)[::-1],0)
            else:raise AssertionError(kind)
            close(key+'/x',rec['x'],x,0)
            if yy is not None:close(key+'/y',rec['y'],yy,1e-10)
            vertices=np.asarray(rec['vertices']) if rec['kind']=='band' else np.column_stack([rec['x'],rec['y']])
            assert np.all((vertices[:,0]>=rec['xlim'][0])&(vertices[:,0]<=rec['xlim'][1])&
                          (vertices[:,1]>=rec['ylim'][0])&(vertices[:,1]<=rec['ylim'][1])),key
        ...
        notes=o.read_json(folder/'annotations.json');assert len(notes)==8
        for note in notes:
            m,p=note['method'],note['point']
            expected=[points.loc[(1168,m,p),'rmse'],points.loc[(1168,c.B,p),'rmse'],
                      100*points.loc[(1168,m,p),'coverage90'],points.loc[(1168,m,p),'width90']]
            close(note['figure']+'/'+p+'/annotation',[note[k] for k in ('rmse','rmse_bplus','coverage90','width90')],expected,0)
            text=''.join(v.text or '' for v in trees[note['figure']].findall('.//s:text',NS))
            assert all(f'{v:.2f}' in text for v in expected[:2]) and f'{expected[2]:.1f}%' in text and f'{expected[3]:.1f}' in text
        ...
        for item in exports['figures']:
            ...
            with Image.open(folder/item['png']) as im:
                ...
                close(name+'/dpi',im.info['dpi'],[300,300],.01,'dpi')
            layout=o.read_json(folder/f'{name}.alignment.json');assert layout['verdict']=='PASS' and layout['auditable']
            assert o.read_json(folder/f'{name}.text_geometry.json')['passed']
            for mode,args in [('text',['audit_pdf_text.py',item['qa_pdf'],'--min-pt','5','--json']),
        ...
        result=subprocess.run([sys.executable,str(scripts/'validate_figure.py'),str(c.ROOT/'code/teacher_refresh/figures.py'),'--json'],capture_output=True,text=True)
        (out/'source_qa.json').write_text(result.stdout)
        assert json.loads(result.stdout)['summary']['counts']['FAIL']==0
        assert len(exports['figures'])==3 and not list(folder.glob('*.pdf'))
        receipt.update(status='passed',figures=3,panels=12,records=len(records),checks=len(checks),
            values=sum(v['values'] for v in checks),all_scientific_marks_inside_axes=True,
            rendered_audits=rendered,visual_review='separate manual record',
            source_warning_review=['PNG300dpi/SVG are the frozen outputs; no TIFF required',
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--attempt',default='figure_qa_v1');parser.add_argument('--figures',default='v1');args=parser.parse_args()
