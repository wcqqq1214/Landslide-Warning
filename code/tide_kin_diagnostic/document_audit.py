...
def main(attempt):
    ...
    o.write_json(out / "receipt.json", dict(status="running"))
    for lock in [
    ...
    assert not points.loc[points.origin == 612, "probability_available"].any()
    assert (
...
if __name__ == "__main__":
    ...
    parser.add_argument("--attempt", default="document_audit_v2")
    attempt = parser.parse_args().attempt
    try:
        main(attempt)
    except Exception:
        c.o.write_json(
            c.ROOT / c.spec()["out"] / attempt / "failure.json",
