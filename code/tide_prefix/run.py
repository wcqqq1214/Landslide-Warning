"""One prefix-input arm, exact original control reuse and complete evaluation."""
...
def fit_one(cfg, teachers, y, forcing, n, end, arm, seed, sc, dataset):
    ...
    original = c.checkpoint_folder(cfg, n, "TiDE_KIN", seed)
    assert np.array_equal(ms, o.load_npz(original / "schedule.npz")["origins"])
...
def train():
    ...
    for n, end in zip(cfg["origins"], cfg["ends"]):
        ...
        if previous is not None:
            ...
            original_sigmas = o.load_npz(prior / "sigmas.npz")
            for method in cfg["controls"] + cfg["reuse_arms"]:
                assert np.array_equal(sigmas[method], original_sigmas[method]), method
...
def score():
    ...
    rows, summaries, sr, sp, daily, pairs, endpoints, differences, distance = (
        [] for _ in range(9)
    )
    for n, end in zip(cfg["origins"], cfg["ends"]):
        folder = root / f"origin_{n}"
        means, seeds = [o.load_npz(folder / (k + ".npz")) for k in ["means", "seeds"]]
        sigmas = o.load_npz(folder / "sigmas.npz") if n != 612 else {}
        by = {}
        for method, mu in means.items():
            ...
            for ident, sm in [("ensemble", mu)] + [
                ...
                for b in cfg["diagnostic_bins"]:
                    sl = slice(b["start"] - 1, b["end"])
                    vals = o.scores(y[n:end][sl], sm[sl], sigma)
                    for p, v in enumerate(vals):
                        distance.append(
                            dict(
                                ...
                                bin=b["name"],
                                **v,
                                bias=float((sm[sl, p] - y[n:end][sl, p]).mean()),
                                probability_available=sigma is not None,
                            )
                        )
            if n == 612:
                continue
            by[method] = o.scores(y[n:end], mu, sigma)
            ...
            for seed, sm in enumerate(seeds[method]):
                values = o.scores(y[n:end], sm, sigma)
                sr.append(
        ...
        for ident, group in [("ensemble", means)] + [
            (str(s), {k: v[s] for k, v in seeds.items()}) for s in cfg["seeds"]
        ]:
            scores_by_arm = {
                a: o.scores(y[n:end], group[a], sigmas[a]) for a in cfg["arms"]
            }
            for point, p in [("average", None)] + [
                (name, i) for i, name in enumerate(cfg["points"])
            ]:
                values = {
                    a: o.summarize(scores_by_arm[a])
                    if p is None
                    else scores_by_arm[a][p]
                    for a in cfg["arms"]
                }
                for metric in [
                    "mae",
                    ...
                    old, new = (
                        values["TiDE_KIN"][metric],
                        values["TiDE_KIN_PREFIX"][metric],
                    )
                    differences.append(
                        dict(
                            origin=n,
    ...
    keys = ["origin", "method", "seed", "bin"]
    metrics = [
        v
        for v in df.select_dtypes(include=np.number).columns
        if v not in keys and v != "n"
    ]
    dist_summary = df.groupby(keys, sort=False)[metrics].mean().reset_index()
    for name, values in [
    ...
    pair = [
        p
        for p in pairs
        if p["candidate"] == "TiDE_KIN_PREFIX" and p["reference"] == "TiDE_KIN"
    ]
    assert len(pair) == 3
    bplus = {
    ...
    outcome = dict(
        ...
        distance_point_rows=len(distance),
        distance_summary_rows=len(dist_summary),
        stable_prefix_mean_vs_kin=all(
            p["mean_pass"] and p["seed_agreement_pass"] for p in pair
        ),
        stable_prefix_joint_vs_kin=all(
            p["joint_pass"] and p["seed_agreement_pass"] for p in pair
        ),
        reused_fits=12,
        ...
        replacement_goal_met=all(p["joint_pass"] for p in bplus["TiDE_KIN_PREFIX"]),
        selection=False,
    )
