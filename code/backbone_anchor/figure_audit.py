...
def main(attempt, figures):
    ...
    receipt=dict(status='running',started_utc=o.utc(),values=0,max_source_difference=0.,max_svg_difference_pt=0.,max_layout_difference_mm=0.,max_png_dpi_difference=0.,new_fits=0,physical_forwards=0)
    checks=[]
    ...
    def close(name,a,b,tol,kind='source'):
        ...
        receipt['values']+=int(a.size);key={'svg':'max_svg_difference_pt','layout':'max_layout_difference_mm','dpi':'max_png_dpi_difference'}.get(kind,'max_source_difference')
    ...
    try:
        ...
        for rec in records:
            ...
            if name=='factorial_overview':
                ...
                if method in ('a_at_gru','a_at_tf','tf_at_raw','tf_at_anchor'):
        ...
        for note in o.read_json(folder/'annotations.json'):
            figure,point,arm=note['figure'],note['point'],note['arm']
        ...
        # Verify fixed-axis truncation disclosure and shared complete ranges.
        annotations=o.read_json(folder/'annotations.json')
        for note in annotations:
            p=cfg['points'].index(note['point']);arm=note['arm'];low,high=note['limits']
            expected_limits=exports['mentor_limits'][p] if note['view']=='mentor' else exports['common_limits'][p]
            close(note['figure']+'/'+note['point']+'/limits',note['limits'],expected_limits,0)
            outside=lambda v:int(np.count_nonzero((v<low)|(v>high)))
            mu=source[arm][:,p]-y[0,p]
            expected=dict(observed_days=outside(y[:,p]-y[0,p]),bplus_days=outside(source['bplus_full'][:,p]-y[0,p]),
                          mean_days=outside(mu),seed_point_days=outside(source[arm+'__seeds'][:,:,p]-y[0,p]))
            for level in [80,95]:
                q=norm.ppf((1+level/100)/2)*source[arm+'__sigma'][p]
                expected[f'band{level}_days']=int(np.count_nonzero((mu-q<low)|(mu+q>high)))
            assert note['clipped']==expected
            if note['view']=='full':assert sum(expected.values())==0
            if note['view']=='mentor':
                assert expected['observed_days']==expected['bplus_days']==expected['mean_days']==expected['seed_point_days']==0
        from PIL import Image
        for item in exports['figures']:
            with Image.open(folder/item['png']) as img:
                assert img.size==(int(240/25.4*300),int(170/25.4*300))
                close(item['name']+'/png_dpi',img.info['dpi'],[300,300],.01,'dpi')
        ...
        receipt.update(status='passed',completed_utc=o.utc(),figures=9,panels=36,plot_records=len(records),
        ...
        for name,tree in trees.items():
            ...
            height=float(tree.attrib['height'].removesuffix('pt'))
            close(name+'/height_mm',height/72*25.4,170,1e-6,'layout')
        assert len(trees)==9 and len(o.read_json(folder/'annotations.json'))==32
