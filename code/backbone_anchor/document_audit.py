...
def main():
    ...
    doc=c.ROOT/'docs/ootang_backbone_anchor_results.v1.0.md';text=doc.read_text()
    ...
    methods=cfg['controls']+cfg['historical_controls']+cfg['arms']
    ...
    for row,n in zip(parsed['factorial'],cfg['origins'][1:]):
        assert row[0]==str(n)
        for s,k in zip(row[1:],['a_at_gru','a_at_tf','tf_at_raw','tf_at_anchor','interaction']):equal(s,factorial.loc[(n,'rmse'),k])
    ...
    for row,p in zip(parsed['final_points'],cfg['points']):
        assert row[0]==p
        for s,(method,k) in zip(row[1:],[(c.B,'rmse'),('G10_ANCHOR_UNIFORM','rmse'),('T10_ANCHOR_UNIFORM','rmse'),('G10_ANCHOR_UNIFORM','coverage90'),('T10_ANCHOR_UNIFORM','coverage90')]):
    ...
    assert f"{(1-score.loc[(1168,'T10_ANCHOR_UNIFORM'),'rmse']/score.loc[(1168,'T00_RAW_UNIFORM'),'rmse'])*100:.2f}%" in text
    for p,method in [('ATU1','T10_ANCHOR_UNIFORM'),('ATU5','T10_ANCHOR_UNIFORM'),('ATU1','G10_ANCHOR_UNIFORM')]:
    ...
    # Independently check narrative directions, not just displayed rounded values.
    for n in cfg['origins'][1:]:
        for raw,anchor in [('G00_RAW_UNIFORM','G10_ANCHOR_UNIFORM'),('T00_RAW_UNIFORM','T10_ANCHOR_UNIFORM')]:
            assert all(score.loc[(n,anchor),m]<score.loc[(n,raw),m] for m in ['mae','rmse'])
        assert all(score.loc[(n,'G10_ANCHOR_UNIFORM'),m]<score.loc[(n,'T10_ANCHOR_UNIFORM'),m] for m in ['mae','rmse','crps'])
    for p in ['ATU1','ATU5']:
        assert point.loc[(1168,'T10_ANCHOR_UNIFORM',p),'mae']>point.loc[(1168,'T00_RAW_UNIFORM',p),'mae']
    assert cfg['parameters']==dict(GRU=1241,TF=1249)
    for row,arm in zip(parsed['design'],cfg['arms']):
        assert row[:3]==[arm,'GRU' if arm.startswith('G') else 'Transformer','开' if cfg['factors'][arm]['anchor'] else '关']
    for a in cfg['arms']:
        assert score.loc[(1168,a),'rmse']>score.loc[(1168,c.B),'rmse']
    assert score.loc[(1168,'OLD_HALF'),'rmse']<min(score.loc[(1168,a),'rmse'] for a in cfg['arms'])
    current_docs=[doc,c.ROOT/'README.md',c.ROOT/'docs/README.md',c.ROOT/'AGENTS.md',
        c.ROOT/'docs/ootang_backbone_anchor_validation.v1.0.md',c.ROOT/cfg['figures']/'README.md']
    links=[]
    ...
    assert rec['new_fits']==24 and rec['optimizer_updates']==4800 and rec['reused_fits']==24 and rec['checkpoints']==192
    assert rec['full_horizon']==293 and rec['all_planned_methods_seeds_dates_completed']
    o.verify_lock(c.ROOT/cfg['figures']/'v1/artifact_lock.json');c.guard()
