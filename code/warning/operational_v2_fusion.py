...
class StationEvidenceResult:
    ...
    # v4 adds acceleration as an independent ordinal evidence family.  These
    # optional fields keep the v1-v3 result shape backward compatible while
    # allowing the v4 run to retain the full acceleration audit trail.
...
def fuse_station_evidence_families_v4(
    *,
    ...
    """Fuse the four v4 ordinal indicators with acceleration as its own family.

    Velocity and tangent angle remain one kinematic family.  Interval and
    acceleration are independent families and can therefore raise the station
    candidate level.  ``delta_v_state`` is retained only as a raw transition
    audit field; it is never a fifth vote and is not required for assessment.
    """
    ...
    supplied = {} if input_statuses is None else dict(input_statuses)
    unknown = sorted(
        set(supplied).difference({*values, "delta_v"})
    )
    if unknown:
        raise ValueError(f"input_statuses has unknown indicators: {unknown}")
    statuses: list[tuple[str, str]] = []
    ...
    if "delta_v" in supplied:
        ...
        if delta_status not in INPUT_STATUSES:
            raise ValueError(
                f"delta_v status must be one of: {', '.join(INPUT_STATUSES)}"
            )
        statuses.append(("delta_v", delta_status))

    nonvalid = next(
        (status for _, status in statuses[:4] if status != "valid"),
        None,
    )
    ...
    raw_delta_state = delta_v_state if isinstance(delta_v_state, str) else None
    if raw_delta_state is not None:
        raw_delta_state = _normalise_delta_v_state(raw_delta_state)
    acceleration_status = (
        "accelerating" if acceleration > WarningLevel.GREEN else "not_accelerating"
    )
    transition_status = (
        _TRANSITION_STATUS_BY_DELTA_V.get(raw_delta_state, "delta_v_not_available")
    )
