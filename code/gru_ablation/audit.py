...
from overnight_graph.audit import independent_inputs, numpy_forward, raw
...
def main(attempt):
    ...
    try:
        ...
        bank = o.bank(cfg); methods = cfg['controls']+cfg['arms']
                ...
                for seed in cfg['seeds']:
                    ...
                    for step in cfg['checkpoints']:
                        ...
                        if step==0:
                            ...
                            init = torch.load(prior/'GRU_GRAPH'/f'seed_{seed}'/'e0.pt',map_location='cpu',weights_only=True)['state_dict']
                            for k in saved['state_dict']:
                                close(f'{n}/{arm}/{seed}/init/{k}',saved['state_dict'][k].numpy(),init[k].numpy(),0)
                        if arm=='G00_RAW_UNIFORM':
                            close(f'{n}/{seed}/{step}/original',saved_mean,np.load(prior/'GRU_GRAPH'/f'seed_{seed}'/f'e{step}_mean.npy'),1e-7)
        ...
        for n in cfg['origins']:
            for seed in cfg['seeds']:
                for left,right in [(cfg['arms'][0],cfg['arms'][1]),(cfg['arms'][2],cfg['arms'][3])]:
                    a,b=[o.load_npz(root/f'origin_{n}'/arm/f'seed_{seed}/schedule.npz') for arm in (left,right)]
        ...
        for dest,source in factorials:
            ...
            for _,row in frame.iterrows():
                ...
                expected=dict(a_at_uniform=v10-v00,a_at_boundary=v11-v01,
                    b_at_raw=v01-v00,b_at_anchor=v11-v10,
                    a_main=(v10+v11-v00-v01)/2,b_main=(v01+v11-v00-v10)/2,
        ...
        assert len(started)==len(completed)==48 and sum(v['updates'] for v in completed)==9600
        identities={(n,a,s) for n in cfg['origins'] for a in cfg['arms'] for s in cfg['seeds']}
        ...
        for i,v in enumerate(events):
            if v['event']=='fit_started':
                prior_reads=[e for e in events[:i] if e['event']=='label_prefix_read']
        ...
        assert receipt['checkpoints']==192 and receipt['original_checkpoints']==48
        assert receipt['fits_verified']==48 and receipt['updates_verified']==9600
        assert len(daily)==28128 and len(endpoints)==1152
        for name,count in [('phase_summary',24),('metrics_by_point',96),('seed_summary',72),('seed_metrics_by_point',288)]:assert len(frames[name])==count
