...
from monitoring import (  # noqa: E402
    ootang_epoch_source_derived_dependent_outcome_dispatch as dispatch,
)
from monitoring import (  # noqa: E402
...
DEFAULT_CONFIG_PATH = (
    ROOT
    / "config"
    / "ootang_epoch_source_derived_dependent_outcome_consumption.v1.json"
)
DEFAULT_CONFIG_SHA256 = (
    "b5169b116116c27cf755913f4a52a3c2ecf68b46213ffb77cc6894013c92eb57"
)
IMPLEMENTATION_LOGICAL_PATH = (
    "code/monitoring/ootang_epoch_source_derived_dependent_outcome_consumption.py"
)
...
EXPECTED_RUNTIME = {
    ...
    "namespace": "source_derived_dependent_outcome_consumption_v1",
    "intents": "intents",
    "receipts": "receipts",
    "events": "events",
    "status": "status.json",
...
EXPECTED_PROTOCOL = {
    "intent_schema_version": "ootang_epoch_source_derived_dependent_outcome_consumption_intent_v1",
    "receipt_schema_version": (
        "ootang_epoch_source_derived_dependent_outcome_consumption_receipt_v1"
    ),
    "event_schema_version": "ootang_epoch_source_derived_dependent_outcome_consumption_event_v1",
    "status_schema_version": (
        "ootang_epoch_source_derived_dependent_outcome_consumption_status_v1"
    ),
    "event_type": "epoch_source_derived_dependent_outcome_consumption_recorded",
    ...
    "readiness_policy": "exact_dependent_dispatch_intent_receipt_event_for_current_overlay_d_or_r",
    "poll_policy": ("heal_or_consume_at_most_one_dependent_materialized_effective_key"),
...
TRUE_CAPABILITIES = (
    ...
    "dependent_materialization_dispatch_deep_verified",
    "dependent_dependency_proof_bound",
    ...
    "source_derived_dependent_outcome_consumption_implemented",
...
class SourceDerivedDependentOutcomeConsumptionError(RuntimeError):
    """Base error for the source-derived dependent outcome consumption bridge."""


class SourceDerivedDependentOutcomeConsumptionConfigError(
    SourceDerivedDependentOutcomeConsumptionError
):
    """The reviewed profile or a pinned dependency changed."""


class SourceDerivedDependentOutcomeConsumptionIntegrityError(
    SourceDerivedDependentOutcomeConsumptionError
):
    """An authority, plan, CAS slice, or durable record failed closed."""


class SourceDerivedDependentOutcomeConsumptionBusyError(
    SourceDerivedDependentOutcomeConsumptionError
):
...
class SourceDerivedDependentOutcomeConsumptionPaths:
    registry_root: Path
    root: Path
    intents: Path
    receipts: Path
    ...
    active_root: Path
    shadow_root: Path
    dispatch: dispatch.SourceDerivedDependentOutcomeDispatchPaths


@dataclass(frozen=True)
class SourceDerivedDependentOutcomeConsumptionResult:
    status: str
    reason: str
    status_path: Path
...
def _canonical_bytes(value: object) -> bytes:
    try:
        ...
        raise SourceDerivedDependentOutcomeConsumptionIntegrityError(
...
def _text(value: object, *, name: str) -> str:
    if not isinstance(value, str) or not value or value.strip() != value:
        raise SourceDerivedDependentOutcomeConsumptionIntegrityError(f"{name} changed")
    return value
...
def _parse_utc(value: object, *, name: str) -> datetime:
    try:
        ...
        raise SourceDerivedDependentOutcomeConsumptionIntegrityError(str(exc)) from exc
...
def _child(root: Path, relative: str, *, name: str) -> Path:
    try:
        ...
        raise SourceDerivedDependentOutcomeConsumptionConfigError(str(exc)) from exc
...
def _strict_json(
    ...
    except dispatch.SourceDerivedDependentOutcomeDispatchError as exc:
        raise SourceDerivedDependentOutcomeConsumptionIntegrityError(str(exc)) from exc
...
def _implementation_reference() -> dict[str, object]:
    try:
        snapshot = registry._read_regular(  # noqa: SLF001
            Path(__file__),
            name="source-derived dependent outcome consumption implementation",
...
def load_source_derived_dependent_outcome_consumption_profile(
    ...
    if root != ROOT.resolve() or resolved.resolve() != DEFAULT_CONFIG_PATH.resolve():
        raise SourceDerivedDependentOutcomeConsumptionConfigError(
            "Only the reviewed default source-derived consumption profile is accepted"
        )
    try:
        snapshot = registry._read_regular(  # noqa: SLF001
            resolved,
            name="source-derived dependent outcome consumption profile",
            maximum_bytes=MAX_CONTROL_BYTES,
        )
        profile = registry._decode_json(  # noqa: SLF001
            snapshot.raw, name="source-derived dependent outcome consumption profile"
        )
    except registry.EpochRegistryError as exc:
        raise SourceDerivedDependentOutcomeConsumptionConfigError(str(exc)) from exc
    ...
    if (
        ...
        != "ootang_epoch_source_derived_dependent_outcome_consumption_profile_v1"
        or profile["profile_id"]
        != "ootang-epoch-source-derived-dependent-outcome-consumption-v1"
        or profile["profile_version"] != "1.0.0-dependent-effective-leaf-terminal"
        or profile["case"] != "ootang"
        or profile["artifact_status"]
        != "narrow_dependent_dispatch_effective_key_outcome_consumption_only"
    ...
    dispatch.load_source_derived_dependent_outcome_dispatch_profile()
...
def source_derived_dependent_outcome_consumption_paths(
    profile: Mapping[str, Any],
    *,
    registry_root: Path | None = None,
    active_root: Path | None = None,
    shadow_root: Path | None = None,
) -> SourceDerivedDependentOutcomeConsumptionPaths:
    registry_path = (
    ...
    dispatch_profile = dispatch.load_source_derived_dependent_outcome_dispatch_profile()
    dispatch_paths = dispatch.source_derived_dependent_outcome_dispatch_paths(
        dispatch_profile,
        registry_root=registry_path,
        active_root=active,
        shadow_root=shadow,
    )
    return SourceDerivedDependentOutcomeConsumptionPaths(
        registry_root=registry_path,
        root=root,
...
def _acquire_locks(
    paths: SourceDerivedDependentOutcomeConsumptionPaths,
) -> list[BinaryIO]:
    try:
        return dispatch._acquire_locks(paths.dispatch)  # noqa: SLF001
    except dispatch.SourceDerivedDependentOutcomeDispatchBusyError as exc:
        raise SourceDerivedDependentOutcomeConsumptionBusyError(str(exc)) from exc
    except dispatch.SourceDerivedDependentOutcomeDispatchError as exc:
        raise SourceDerivedDependentOutcomeConsumptionIntegrityError(str(exc)) from exc
...
def _previous_step_adapter(receipt: Mapping[str, Any]) -> dict[str, object]:
    try:
        return source_consumption._previous_step_adapter(receipt)  # noqa: SLF001
    except source_consumption.SourceDerivedOutcomeConsumptionError as exc:
        raise SourceDerivedDependentOutcomeConsumptionIntegrityError(str(exc)) from exc


def _step_id(
    overlay_event_sha256: str,
    dependent_event_sha256: str,
    dependency_proof_sha256: str,
    key_id: str,
) -> str:
    return _sha256(
        _canonical_bytes(
            {
                ...
                "dependent_dispatch_event_sha256": dependent_event_sha256,
...
def _materialized_candidates(
    candidates: Sequence[dispatch._DependentCandidate],  # noqa: SLF001
    ...
    for event, event_snapshot in events:
        ...
        if candidate is None:
            raise SourceDerivedDependentOutcomeConsumptionIntegrityError(
                "Materialization event escaped the exact dependent D/R set"
            )
        item = candidate.item
        if (
        ...
        proof = materialization_intent.get("dependency_proof")
        proof_sha256 = materialization_intent.get("dependency_proof_sha256")
        if (
            not isinstance(proof, dict)
            or proof != dict(candidate.dependency_proof)
            or proof_sha256 != _sha256(_canonical_bytes(proof))
        ):
            raise SourceDerivedDependentOutcomeConsumptionIntegrityError(
                "Dependent dispatcher dependency proof changed"
            )
        result.append(
            _MaterializedCandidate(
                ...
                dependency_proof_sha256=str(proof_sha256),
...
def _intent_payload(
    ...
    *,
    ...
    step_id = _step_id(
        ...
        candidate.dependency_proof_sha256,
        str(item["key_id"]),
    )
    ...
    return {
        ...
        "overlay_event": _reference(
            overlay_event, paths.dispatch.dispatch.overlay.root
        ),
        "dependent_dispatch": {
        ...
        "dependency_proof": dict(candidate.dependency_proof),
        "dependency_proof_sha256": candidate.dependency_proof_sha256,
...
def _receipt_payload(
    ...
    *,
    ...
    return {
        ...
        "dependent_dispatch_event": intent["dependent_dispatch"]["event"],
...
def _event_payload(
    ...
    *,
    ...
    body = {
        ...
        "dependent_dispatch_event": receipt["dependent_dispatch_event"],
...
def _publish_intent(
    paths: SourceDerivedDependentOutcomeConsumptionPaths, payload: Mapping[str, Any]
) -> registry.ArtifactSnapshot:
...
def _result(
    ...
    *,
    ...
    receipt_path: Path | None = None,
    event_path: Path | None = None,
) -> SourceDerivedDependentOutcomeConsumptionResult:
    consumed = event_path is not None
    return SourceDerivedDependentOutcomeConsumptionResult(
        status=status,
        reason=reason,
...
def _coordinate_source_derived_dependent_outcome_consumption(
    *,
    ...
    profile = load_source_derived_dependent_outcome_consumption_profile(config_path)
    paths = source_derived_dependent_outcome_consumption_paths(
        profile,
    ...
    try:
        ...
        try:
            computation, waiting = overlay._load_computation(  # noqa: SLF001
                overlay_profile,
                paths.dispatch.dispatch.overlay,
                now,
        ...
        try:
            stored_overlay, published_overlay = overlay._load_overlay_state(  # noqa: SLF001
                overlay_profile, paths.dispatch.dispatch.overlay, computation
            )
        except overlay.SourceDerivedWorksetOverlayError as exc:
            raise SourceDerivedDependentOutcomeConsumptionIntegrityError(
                str(exc)
        ...
        try:
            dependency_receipts, dependency_events = (
                dispatch._deep_replay_prerequisites(  # noqa: SLF001
                    computation, overlay_event_snapshot, paths.dispatch, now=now
                )
            )
            terminals = dispatch._terminal_consumption_dependencies(  # noqa: SLF001
                computation,
                dependency_receipts,
                dependency_events,
                paths.dispatch,
            )
            candidates = dispatch._dependent_candidates(  # noqa: SLF001
                computation, terminals, paths.dispatch
            )
            dispatch_profile = (
                dispatch.load_source_derived_dependent_outcome_dispatch_profile()
            )
            (
                ...
                candidates,
                now=now,
                build_plan=dispatch._build_plan,  # noqa: SLF001
        ...
        if selected is None:
            if materialized:
                status = "source_derived_dependent_outcome_consumption_current"
        ...
        current_step = _step_id(
            ...
            selected.dependency_proof_sha256,
    ...
    except SourceDerivedDependentOutcomeConsumptionBusyError:
        raise
    except SourceDerivedDependentOutcomeConsumptionError as exc:
        if handles:
            try:
                _write_status(
                    profile,
            ...
            except SourceDerivedDependentOutcomeConsumptionError:
                pass
        raise
...
def coordinate_source_derived_dependent_outcome_consumption(
    *, config_path: Path = DEFAULT_CONFIG_PATH
) -> SourceDerivedDependentOutcomeConsumptionResult:
    """Run one machine-only source-derived dependent outcome consumption poll."""

    return _coordinate_source_derived_dependent_outcome_consumption(
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = coordinate_source_derived_dependent_outcome_consumption(
