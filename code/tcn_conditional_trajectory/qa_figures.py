...
def main():
    ...
    assert (
        sha(ROOT / "code/tcn_conditional_trajectory/figures.py")
    ...
    for name in [*cfg["arms"], "all_methods_forecast"]:
        ...
        for p, point in enumerate(cfg["points"]):
            ...
            if name in cfg["arms"]:
                ...
                assert f"TCN {a:.2f} mm" in text and f"B+ {b:.2f} mm" in text
                np.testing.assert_allclose(
    ...
    receipt = dict(
        ...
        static_preflight="PDF requirements overridden by user; callable alignment alias and width expression not recognized by static checker; actual rendered geometry checked",
        visual_inspection="all three complete PNGs and all 12 panels inspected in this task; no clipping, missing glyphs or annotation overlap observed",
    )
