"""Two mentor-style figures for the fixed residual-penalty experiment."""
...
COLORS = {
    "TRANSFORMER_BRES_REG1": "#246592",
    "BPLUS_CONTINUOUS": "#777777",
    "DRIFT1": "#B17C46",
    "RR_COND": "#8A7FAD",
    "TCN_DIRECT_COND": "#246592",
    "TCN_BRES_COND": "#70A3B6",
    "TRANSFORMER_DIRECT_COND": "#E69F00",
    "TRANSFORMER_BRES_COND": "#C55255",
    "CNN_MAMBA_DIRECT_COND": "#756CB0",
    "CNN_MAMBA_BRES_COND": "#00856D",
}
NAMES = {
    "TRANSFORMER_BRES_REG1": "Transformer 正则化",
    "BPLUS_CONTINUOUS": "改进 B+",
    "DRIFT1": "DRIFT1",
    "RR_COND": "普通岭回归",
    "TCN_DIRECT_COND": "TCN 直接",
    "TCN_BRES_COND": "TCN 残差",
    "TRANSFORMER_DIRECT_COND": "Transformer 直接",
    "TRANSFORMER_BRES_COND": "Transformer 原版",
    "CNN_MAMBA_DIRECT_COND": "CNN-Mamba 直接",
    "CNN_MAMBA_BRES_COND": "CNN-Mamba 残差",
}
DOMAINS = ("O3", "O2", "O1-up", "O1-down")


def comparison_style(method):
    if method == "BPLUS_CONTINUOUS":
        return "--"
    if method == "TCN_DIRECT_COND":
        return "-."
    if method == "TCN_BRES_COND":
        return ":"
    if "_DIRECT_" in method:
        return (0, (4, 1.5))
    return "-"
...
def save(fig, path, require_matplotlib_panel_alignment):
...
def main(attempt):
    ...
    data = load_npz(
        ROOT / cfg["reuse"] / "implementation_verification/teacher_1168.npz"
    )
    ...
    for arm in cfg["arms"]:
        fig, axes = canvas(
            f"{NAMES[arm]}：四点训练拟合与 8:2 独立条件预测",
        ...
        for p, (ax, point) in enumerate(zip(axes.flat, cfg["points"])):
            ...
            ax.text(
                0,
                -0.215,
                f"预测 RMSE：模型 {model_rmse:.2f} mm；B+ {b_rmse:.2f} mm",
    ...
    fig, axes = canvas(
        "四点完整 293 日预测：Transformer 正则化验证",
        "五种固定方法，同一完整日期｜均值对比；正则版区间见配套图",
    )
    ...
    fig.text(
        0.5,
        0.044,
        "Transformer 主线均为三个种子均值；原版与基线复用同协议结果，未重新训练。",
        ha="center",
        fontsize=7.6,
    )
