    ...
    for model in ("M1", "M2"):
            ...
            for phase in ("train", "prediction"):
                for metric in ("rmse", "mae"):
                    ...
                    if (
                        len(m) != 1
                        or len(b) != 1
                        or not np.isfinite(m.value.iloc[0])
                        or not np.isfinite(b.value.iloc[0])
                    ):
        ...
        result[model] = dict(
            ...
            evaluation_status=(
                "evaluated"
                if all(
                    all(v is not None for v in point["differences"].values())
                    for point in flags
                )
                else "not_evaluated"
            ),
...
def plot_results(predictions, observed, drivers, output):
    ...
    for j, point in enumerate(POINTS):
        ...
        zoom_values = [observed[1168:, j]]
        residual_values = [np.zeros(1)]
        for model in ("M0", "M1", "M2"):
            ...
            if frame.empty or not np.isfinite(frame.mean_mm.to_numpy()).any():
                continue
            mu = frame.mean_mm.to_numpy()
            zoom_values.append(mu[1168:])
            residual_values.append(mu[1168:] - observed[1168:, j])
            for ax in axes[:2]:
            ...
            if model != "M0":
                for side in ("lower_90_mm", "upper_90_mm"):
                    bounds = frame[side].to_numpy()[1168:]
                    zoom_values.append(bounds)
                    residual_values.append(bounds - observed[1168:, j])
                axes[2].fill_between(
                    dates,
        ...
        for ax, values in zip(axes[1:], [zoom_values, residual_values]):
            low = min(float(np.nanmin(v)) for v in values)
            high = max(float(np.nanmax(v)) for v in values)
