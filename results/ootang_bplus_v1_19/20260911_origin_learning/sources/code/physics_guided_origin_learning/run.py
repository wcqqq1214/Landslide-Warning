"""One finite paired origin-learning run with prefix and probability locks."""
...
def execute(out, state):
    ...
    paths = [
        *sorted((ROOT / "code/physics_guided_origin_learning").glob("*.py")),
        ROOT / "tests/test_physics_guided_origin_learning.py",
        CONFIG,
        ROOT / spec["protocol"],
    ]
            ...
            for strategy in ("IN", "OOF"):
                ...
                for seed in spec["seeds"]:
                    ...
                    for epoch in range(1, spec["epochs"] + 1):
                        ...
                        loss, norm = mean_step(model, op, inputs[strategy])
                    ...
                    final = final_losses(model, inputs[strategy])
                    summaries.append(
                        dict(
                            fit_days=h,
                            strategy=strategy,
                    ...
                    means.append(
                        predict(model, base, features, labels, h, *scalers, "H")
                    )
                    print(
    ...
    if state != dict(neural_updates=1800, scale_fits=6):
...
def main():
    ...
    spec = specification()
    out = ROOT / "results/ootang_bplus_v1_19" / args.run_id
    if args.verify:
        from .verify import verify
