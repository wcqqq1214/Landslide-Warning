...
| 文件 | 作用 | 类型 | 论文用途 |
| --- | --- | --- | --- |
| `pipeline/latest_run.json` | 保存统一入口最近一次实际运行的提交哈希、源码指纹、Python 版本、阶段契约状态、退出码、耗时及输出 SHA-256 | 工程验收清单 | 由当前分支重新运行后生成；证明管线执行范围、产物完整性和失败点，不作为模型性能证据 |
| `pipeline/shap_stability_run.json` | 保存修复后 SHAP 稳定性单阶段正式运行的源码指纹、耗时和 9 个产物哈希 | 单阶段工程清单 | 追溯正式结果到提交 `3c06d38`；不替代完整管线清单 |
| `cnn_mamba/forecast_interval.png` | 展示 8 个测点的一日位移 P10/P50/P90 预测区间 | 最终图 | 位移预测主图；需在 WSL/Linux + NVIDIA CUDA 环境重新生成 |
| `cnn_mamba/forecast_metrics.csv` | 保存各测点校准前后的 RMSE、MAE、R2/NSE、持久性基线、pinball loss、覆盖率、宽度和 80% interval score | 最终评估表 | 生成位移预测结果表；用 `interval_variant` 区分原始和校准区间，R2/NSE 仅作补充 |
| `cnn_mamba/forecast_period_metrics.csv` | 将 287 日测试段按日期连续分为三个块并保存校准前后同组指标 | 时间稳定性审计表 | 检查总体均值是否掩盖后期性能退化，不代替滚动时间验证 |
| `cnn_mamba/forecast_calibration_metrics.csv` | 保存拟合/校准/测试日期边界、测点独立 `qhat` 及校准前后覆盖率、宽度、pinball 和 interval score | 校准审计表 | 证明校准期早于测试期并量化宽度-覆盖率代价；不提供时间序列下的严格覆盖保证 |
| `cnn_mamba/forecast_bootstrap_ci.csv` | 保存总体和各测点在 7/14/30 日连续块下的点估计、95% 百分位区间、配对差值及完整重采样参数 | 不确定性审计表 | 14 日为主分析，7/30 日为敏感性；模型和 `qhat` 固定，不能解释为训练或未来漂移不确定性 |
| `cnn_mamba/rolling_validation_*.csv` | 保存三个扩展窗口折的协议、逐折指标和逐日预测 | 滚动验证审计表 | 必须逐折报告，不以平均值掩盖时期差异 |
| `cnn_mamba/seed_stability_*.csv` | 保存种子 0-4 的运行协议、逐种子指标、跨种子汇总和训练轨迹 | 多种子诊断表 | 逐次报告初始化波动，不以跨种子均值替代失败运行 |
| `cnn_mamba/inner_validation_*.csv` | 保存内层 epoch 选择、最终重训、外层指标、逐日预测和固定 120 轮配对比较 | 早停诊断表 | 证明 epoch 仅由拟合期内部信息选择，未选择最佳种子 |
| `cnn_mamba/capacity_*.csv` | 保存预注册容量/正则化候选、选择过程、最终运行和配对比较 | 容量/正则化诊断表 | 同时报告改善与退化，不按外层结果扩展搜索 |
...
- 当前 Mamba 分支不保留旧位移预测快照；CNN-Mamba 结果应由当前分支重新运行生成并追溯到新的 Git 提交。
