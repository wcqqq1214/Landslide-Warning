...
用户随后指定联合均值/尺度概率 ConvLSTM，本次先交付 v2.4 计划，未实现或训练；
保留同条件梯度隔离对照，只检验一个联合反传候选，不恢复已停止的 PINN。
...
| [v2.4 联合概率 ConvLSTM 计划](ootang_convlstm_joint_plan.v2.4.md) | 用户指定候选的 plan；一个联合反传候选与必要配对对照、固定同窗判据和拟议预算；尚未实现/运行 |
