...
**当前：先写导师条件下的 TCN 重新训练计划。** [新训练计划](ootang_tcn_conditional_training_plan.v1.0.md)：给定未来降雨/库水位、原 B+ 连续状态、完整 8:2 切分，从头训练两臂整段轨迹；目前仅文档，未实现或训练。原两轮均已完成，不恢复旧预算，不自动加入 Transformer/Mamba。
