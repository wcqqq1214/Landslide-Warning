...
| [`operational_run.py`](../code/warning/operational_run.py) / `figures/warning_operational_draft/ootang_operational_run_manifest.json` | 导师复核用、可替换的藕塘四指标实施版；以 JSON 范围表和数值字段执行 fit-only KMeans 对照基线、`V0±σ`、选段 `ΔV` MAD 与透明支撑规则，锁定基础 `1.3-draft` 指纹/未决项。每份 CSV/manifest 固定 `operational_draft_not_formal`、`formal_warning_output=false`、`vajont_used=false`，可捕获的提升失败回滚旧快照。 | 先完整重跑藕塘、检查每点和多点输出，并在导师意见后替换版本化运行配置。 | 称为正式预警、称 KMeans 为 Word-MVIF `V0`、用 test 期反选参数、将 `uncorroborated` 并入 green，或启动 Vajont。 |
