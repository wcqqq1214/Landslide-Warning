...
> 工程收口（2026-08-11）：历史路径继续保留用于复现和边界说明，不进入默认入口。统一入口现为 `features → convlstm → ootang-operational-v4` 三阶段最小链，v3 为显式复现对照；schema 3 清单另行记录逐阶段输入/输出指纹。本文中的旧运行清单和旧测试数字按各自日期解释，不能当作当前门禁；当前全量门禁为 367 tests。Vajont 未启动。
...
| [`operational_v2_fusion.py`](../code/warning/operational_v2_fusion.py) / `figures/warning_operational_draft_v2/ootang_operational_run_manifest.json` | v2 非监督空间证据族实施版；将速度/切线角作为一个运动学证据族，`ΔV` 只标记加速性。O1/O2/O3 拓扑由 Wang 等（2025，DOI `10.1029/2025JH000592`）PDF 第 7 页图 4(a,d) 与 5.2 节提供来源；profile 锁定已审查指纹，重建后的 manifest 会记录本地副本核验状态，当前历史 v2 快照尚未刷新该字段。blue 仅在最高候选为 blue 时可见，yellow--red 采用跨区支撑且未确认者不降为 blue。全局最少 3 点门禁缺陷已修复，当前完整数据快照数值不变。每份 CSV/manifest 固定 `operational_draft_not_formal`、`formal_warning_output=false`、`vajont_used=false`。 | 审计测点候选、空间覆盖与跨区确认，和 v1 对照规则影响。 | 称为指定 Word 的 Logistic 融合或正式 `F/F_site`，将候选未确认状态当作 green/传感器缺失，调参使用 test，或启动 Vajont。 |
| [`operational_v3_fusion.py`](../code/warning/operational_v3_fusion.py) / `figures/warning_operational_draft_v3/ootang_operational_run_manifest.json` | v3 双轴非监督空间草案；任何 site 色先要求至少 3 点及 O1/O2/O3 全覆盖，blue 也要求 2 点/2 区，同时保留 `site_confirmed_level`、`local_max_candidate_level` 与 `localized_blue_attention`。独立目录和源码指纹避免覆盖 v2。 | 审计整体跨区确认与局部最高候选的差别，并作为导师要求的藕塘内部原型。 | 把 green 当作已验证安全、把 400 个未确认候选当作误报、称为指定 Word Logistic/正式 `F_site`，或启动 Vajont。 |
