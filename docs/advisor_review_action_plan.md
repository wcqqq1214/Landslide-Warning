...
> **历史／非权威长记录。** 本文保留导师意见落实过程与旧阶段决策用于溯源，不再作为当前科研主线、实现状态或结果口径；当前权威入口见 [`docs/README.md`](README.md)。
...
> **科研主线纠偏（2026-08-30，覆盖下文较早决策）**：用户没有同意以透明规则永久替代
> 导师要求的 NGBoost。NGBoost 的目标 `y` 可以由机器从未来变形状态自动生成，不要求人工逐时刻
> 标注；自动标签器的参数、状态中心和边界只能由开发历史学习，不能查看最终留出折。当前主线为
> `{置信区间, 逐点速度, 严格逐点加速度, 改进切线角} → 自动未来状态标签 → NGBoost 五分类 →
> 8 点逐时刻与滑坡体综合输出`。v4 透明规则保留为可解释基线，不作为训练标签。独立现场/事件真值
> 仍是确认性灾害效能主张的必要条件，但不阻塞自动化科研原型；formal-v5 G1--G4 只约束前者。
> 下文凡称“本轮采用透明规则替代 NGBoost”“必须取得人工/独立标签后才能训练”或把 `ΔV` 列为
> 当前第四指标的段落，均以本纠偏为准；`ΔV` 仅作论文来源说明和审计字段。

> **方法对齐审计（2026-08-31）**：指定 Word 第五章没有严格加速度阈值表；表 5-4 是速度
> `V0/5V0/10V0`，第四项是 `ΔV`。当前 fit-only `A0=max(1.5A,A+2σa)` 五级是导师“同阈值
> 结构”要求下的项目操作化，不是论文原阈值。NGBoost 的 `y` 是 H=7 未来位移率与未来正速度
> Q90 的自动多点代理状态；严格加速度与其余三指标是 `X_t`，不得以同刻融合色制造
> `y=F(X_t)` 循环。SHAP 解释独立 NGBoost，不是 ConvLSTM 内部。要求的输入、八点全时间、
> 五分类逐时五色与多点综合已基本实现，但分类器未胜 persistence，仍为 rejected exploratory。
> 旧 fold 3 指标只作历史产物，不重跑或级联改写；后续 fold 3 仅预测。本审计不设置人工批准
> 或冻结，也不修改标签或模型。
...
4. 以机器自动生成的未来变形状态为代理标签训练 NGBoost 五分类，逐时刻输出单测点和多测点综合状态；v4 五级透明规则只作基线；
...
| --- | --- | --- | --- |
| 藕塘滑坡 8 个测点 | 当前唯一重算主线 | 复用时间 OOF 预测，构建四指标、自动未来状态标签、NGBoost 概率分类、SHAP 与多测点逐时刻输出；无现场真值时只称自动代理状态原型 | 是 |
...
| 已确认 | R5：以自动未来变形状态为代理标签训练 NGBoost 五分类；v4 透明规则为基线 | 自动标签器只在开发历史上学习，不读取最终留出折，不以 v4 同时刻规则颜色自标；独立现场标签仅用于确认性效能升级 |
...
| R5 | 采用 NGBoost 对预警指标分类训练 | 用户要求系统机器自动运行，不采用人工逐时刻标注 | 以开发期自动未来状态为代理标签训练五分类；v4 规则仅作基线，独立现场标签只用于后续确认性验证 |
...
| R2 | ConvLSTM 展示所有测点，并同时显示训练段和预测段 | 结果完整性 / P1 | 已实现 | 保持 8 测点全时间轴图及 fit、calibration、test/prediction 边界 | 藕塘 8 个测点全部出现；图和逐时刻预测 CSV 可互相核对 |
| R3 | 明确 ConvLSTM 与 SHAP 分别用于区间覆盖和主控因素分析 | 方法解释 / P0 | 已实现为探索性解释 | ConvLSTM 输出 `P10/P50/P90` 并评价覆盖；独立 NGBoost 概率模型的 SHAP 只分析模型依赖 | 不将 SHAP 写成 ConvLSTM 内部解释、因果证据或已确认的物理主控因素 |
...
| R5 | 采用 NGBoost 分类训练 | 概率分类 / P1 | 科研主线 | 以自动未来变形状态为代理标签，输入区间、速度、严格加速度和改进切线角；v4 规则作为基线 | 逐时刻输出五类概率和颜色；自动标签、时间折、8 点贡献均可追溯，不把代理状态写成现场灾害真值 |
...
### 3.2 ConvLSTM 三测点旧图缺口已解决

- 历史实现曾只画 `MJ9/MJ1/MJ3` 的测试段；当前主结果已覆盖 8 个测点和完整时间轴，不再把“三测点图”列为待办。
- 图与逐时刻预测 CSV 保持可核对，并继续区分 fit、calibration 与 prediction/test 边界。
- 训练段拟合只能作为拟合诊断，不能与留出段性能混合汇报。

### 3.3 SHAP 并不直接解释 ConvLSTM

- 历史 `shap_select.py` 曾同时训练 NGBoost 回归器和 30 日 V0 分类器；该脚本已移至 Git 历史。
- 当前五分类原型用独立 NGBoost 概率模型的 permutation SHAP 解释期望等级；它不是 ConvLSTM 的 SHAP。
- 用户已确认采用“解释模型与概率预测模型分离”的角色。ConvLSTM 负责 `P10/P50/P90` 和覆盖评价，独立 NGBoost+SHAP 只描述候选模型依赖，不能当作物理因果主控因素。
...
**不要混淆“速度自身的五级状态”和“最终预警等级”**：表 5-4 只定义速度这一项的状态；第五章 5.2.3 再将多指标融合为最终五级。当前项目按导师最新意见以区间、速度、严格加速度和切线角输入 NGBoost；`V0` 或任何单一指标都不能独自决定最终颜色。v4 规则输出只作对照，NGBoost 标签来自未来原始变形状态而非同一时刻四指标颜色。
...
2. 导师当前要求采用严格逐点加速度，而不是把 `ΔV` 当作第四项模型输入；`ΔV` 仅保留为论文原始方法差异和运动学审计字段。加速度采用 `a_i=(v_i-v_{i-1})/(t_i-t_{i-1})`，其阈值/状态仍须标明是项目操作化而非论文直接给出的数值表。
3. 参考论文用多项式逻辑回归，本项目按导师意见改用 NGBoost。只能表述为“参考论文的多指标及五级概率融合思想，并以 NGBoost 实现项目模型”，不能宣称复现原论文的具体分类器。
...
- **四指标**：区间、速度、严格逐点加速度 `a_i=(v_i-v_{i-1})/(t_i-t_{i-1})`、改进切线角并列输入，不设主副指标；`ΔV` 只作审计；
...
- **融合路径**：以机器自动生成的未来状态代理标签训练 NGBoost 五分类；透明规则作为独立基线，不给 NGBoost 自标；
...
这些决定已经足以开始四指标 OOF、自动未来状态标签、NGBoost、全测点输出和 SHAP 说明的开发；不应再把它们列为“等待导师确认”。

### 4.2 机器记录的执行协议：不是人工冻结或新的导师问题

| 项目 | 必须写清的内容 | 机器协议要求 |
| --- | --- | --- |
| 速度五级边界 | `V0` 的计算窗口、五级边界及 blue 的精确边界归属 | 依据参考论文的速度框架写入配置；不得沿用旧 30 日位移增量 |
| 严格加速度 | `a_i=(v_i-v_{i-1})/Δt_i`、`mm/day²`、暖启动与缺失状态 | 复用已确认逐点公式；模型使用连续值，`ΔV` 仅作审计且不重复入模 |
| 区间五级映射 | 图 5-1 的 `μ`、`μ+σ`、`μ+2σ`、`μ+3σ` 分界；`μ_t`、`σ_t` 的项目近似、fit 行处置及无效分位数处理 | 校准诊断只读 calibration，作为正态近似的审计；不设置论文未给出的通过阈值，测试期不得据结果调整映射 |
| 自动未来状态标签 | H=7 未来位移率、未来正速度 Q90、fold-1 ECDF 与固定五级边界 | 只由 fold 1 学习并固定应用；严格加速度属于 `X_t`，不进入 `y`；fold 3 后续仅预测 |
| NGBoost 与多测点综合 | 8 点四指标输入、测点/site 目标、五类概率、基线和缺失处理 | 每个 site 结果可回溯到当日全部有效测点；v4 规则只作基线 |
| 数据切分与表述 | 日期分组切分、自动标签版本和代理结局的评价边界 | 不用最终留出折挑方法；不把代理状态性能写成现场灾害效能 |
...
#### 4.2.16 v4 严格逐点加速度扩展（2026-08-11；阈值来源澄清于 2026-08-13）

v4 在不覆盖 v3 核心数值 CSV 的前提下，复用其双轴空间逻辑并增加严格逐点加速度。`code/features/kinematics.py` 保留真实 `dt_days`、三点暖启动和 raw `delta_v` 审计；`code/warning/operational_v4_fusion.py` 是版本化入口，实际融合把 interval、`kinematic_velocity_tangent`（velocity+tangent）和 acceleration 作为三族取局部最大等级。2026-08-13 的来源澄清将指定 Word 的速度 `V0` 基线和 `1×/5×/10×` 五级相对结构以 `A0` 量纲一致地转置到加速度；Word 本身没有严格加速度阈值表。每个时刻的四项输入均需有效，否则测点结果明确 fail-closed。
