...
导师确认加速度沿用逐点导数方法：`a_i=(v_i-v_{i-1})/(t_i-t_{i-1})`，单位 `mm/day²`，使用真实相邻时间差，前三个观测点形成两行暖启动。用户随后明确授权本轮“阈值也取相同的”；实施为在 fit-only 稳定段计算 `A=mean(a)`、样本 `sigma_a`，并以 `A0=max(1.5A,A+2sigma_a)` 复用速度相对带的五级边界：`green a<A0-sigma_a`、`blue [A0-sigma_a,A0+sigma_a]`、`yellow (A0+sigma_a,5A0)`、`orange [5A0,10A0)`、`red >=10A0`。该相对带是用户批准的项目操作假设，不是导师或指定 Word 论文提供的藕塘数值阈值；A0 非有限或不大于零时 fail-closed。
...
| R4/R7 | 使用“加速度”作为指标并设置等级 | 历史 v1/v2/v3 按参考论文术语采用 `ΔV` 三态；2026-08-11 v4 按导师确认恢复严格逐点加速度，阈值相对带为用户批准假设 | v1/v2/v3 以 `ΔV_i=v_i-v_{i-1}` 作过程趋势；v4 使用 `a_i=(v_i-v_{i-1})/Δt_i`、`A0=max(1.5A,A+2sigma_a)` 的独立五级 family，不把阈值写成 Word 原文结论 |
...
| R7 | 为加速度设置预警等级，并参考指定论文 | 方法 / P0 | 已调整并确认 | v1/v2/v3 按论文术语采用 `ΔV` 三态；v4 按导师确认逐点导数方法，五级相对带由用户授权 | 加速度公式、单位、A0 基线、三点暖启动、失效策略及其在 v4 `F` 中的参与方式写入 v2 扩展协议；不伪造论文未给出的现场加速度阈值 |
