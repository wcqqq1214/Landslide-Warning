...
> **工程收口（2026-08-11）**：代码库整体审查已完成并按 Git 提交记录。无参数入口现在只运行 `features → convlstm → ootang-operational-v4`，v3 保留为 explicit-only 数值对照，其余历史复现、v1/v2 和诊断阶段同样为 explicit-only；v4 逐点加速度与五级相对带仍是 prototype/non-formal。入口清单采用 schema 3，记录逐阶段输入/输出文件指纹和工作树状态。该工程门禁不改变数据门禁或证据等级；`latest_run.json` 等未带当前提交的清单仍须按其自身提交和源码指纹解释。
...
forecast_predictions.csv + ootang_kinematics_long.csv
  -> warning/operational_run.py
       ...
       -> warning_operational_draft_v4/ (逐点加速度/双轴空间原型)
...
| `code/warning/operational_run.py` | 校验基础草案、fit-only 参数、预测/拓扑指纹，构建非正式逐点与滑坡体时间线，并隔离 v1/v2/v3/v4 目录；v3/v4 还记录 profile 锁定的拓扑来源摘要和实现源码指纹 | `figures/warning_operational_draft{,_v2,_v3,_v4}/*`；均非正式。Wang 论文 PDF 是可选来源证据副本：本地存在时必须与锁定摘要匹配，缺失时不阻塞计算但须在清单中保留未核验状态 |
...
| `code/warning/operational_v4_fusion.py` | 版本化导出 v4 测点融合入口；速度/切线角保持 `kinematic_velocity_tangent` 单一 family，加速度作为独立 ordinal family，`delta_v` 仅审计 | v4 测点与滑坡体审计记录；不构成正式预警 |
