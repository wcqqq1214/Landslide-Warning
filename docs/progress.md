...
- 位移预测泛化核查（2026-09-01）：当前任务使用过去 7 日预测下一日，并在每个测试时刻以真实
  ...
  未新增调参或训练。
- 位移图诊断重构（2026-09-01）：沿用既有 `forecast_predictions.csv` 免训练重绘全部 8 个测点，
  每点仍为一页。上栏将拟合、校准和测试 P50 统一为一种样式，并用浅色背景标记时间分段；中栏
  集中展示测试段实测/P50 日增量及 P10--P90 区间；下栏展示 `实测-P50` 残差及相对 P50 的
  区间带，残差位于色带内即表示该时刻被区间覆盖。该调整只改变证据呈现，不改数据、模型、
  时间划分、区间、指标或实验结论，也未读取或运行 Vajont。
