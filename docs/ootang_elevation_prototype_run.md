...
- 高程能够改善预测的证据；
- 当前 7 通道模型的滚动、多种子、早停和容量稳定性复验；
- 来源清楚的确认性预测；
- 正式 `V0`、正式融合或工程预警；
- 任何 Vajont 结果。

典型状态日、400 条未空间确认状态和高程可信性已经完成专家审查，见[`藕塘高程通道与空间预警结果专家审查`](ootang_elevation_warning_expert_review.md)。审查确认 400 日全部数据完整且候选只位于 O1，同时发现 v2 有效点门禁没有覆盖非绿色分支、green/blue 的滑坡体语义过严。下一步先修复覆盖门禁并形成不覆盖 v2 的 v3 空间规则草案，不扩大同一 test 段上的模型搜索。若最终论文更换数据集，则以新数据重新建立来源、切分、阈值和验证协议。
