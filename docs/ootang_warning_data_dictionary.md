...
> 工程口径（2026-08-11；阈值来源澄清于 2026-08-13）：本字典仍是 `draft` 数据契约，不授权正式预警。默认入口为 `features → convlstm → ootang-operational-v4`；v3 数值快照保留为对照。导师确认加速度使用指定 Word 的速度 `V0` 相对阈值结构；Word 没有严格加速度阈值表，因此 v4 以自身 `A0` 作量纲一致转换，而不把它写成 Word 已给出的藕塘加速度数值。schema 3 管线清单记录阶段输入/输出指纹，Vajont 仍未启动。
...
- 2026-08-11 新增 [`ootang_operational_run.v4.draft.json`](../config/ootang_operational_run.v4.draft.json) 与 [`operational_v4_fusion.py`](../code/warning/operational_v4_fusion.py)，并于 2026-08-13 澄清阈值来源。v4 逐测点计算 `v_i=(U_i-U_{i-1})/Δt_i`、`a_i=(v_i-v_{i-1})/Δt_i`，单位分别为 `mm/day`、`mm/day²`，并在 fit-only 候选稳定段上计算 `A`、`sigma_a`、`A0`。导师确认加速度沿用指定 Word 的速度 `V0` 相对阈值结构：用 `A0` 量纲一致地对应 `V0`，保留 `1×/5×/10×` 倍数；`A0±sigma_a` 是对 Word 定性 blue 边界的项目操作化。加速度是独立五级 evidence family；速度与切线角仍合并为一个运动学 family；原始 `delta_v` 仅作审计字段。v4 同时锁定 v1 基础协议和 v2 扩展协议的 SHA-256，核心与三类图件 manifest 还复制实现源码指纹、输出行数/哈希和双协议声明；输出目录为 `figures/warning_operational_draft_v4/`，不覆盖 v3 数值快照。Word 未给出严格加速度阈值表，且加速度阈值尚未现场独立验证。
...
| --- | --- | --- | --- |
| `acceleration` | `a_i=(v_i-v_{i-1})/(t_i-t_{i-1})`，单位 `mm/day²`，使用当前速度对应的真实 `Δt_i` | `A=mean(a)`、`sigma_a=std(a,ddof=1)`、`A0=max(1.5A,A+2sigma_a)`；green `<A0-sigma_a`，blue `A0-sigma_a…A0+sigma_a`，yellow `A0+sigma_a…5A0`，orange `5A0…10A0`，red `≥10A0`，边界归属以 v2 JSON 为准 | 导师确认沿用 Word 的速度 `V0` 相对结构；因 Word 未给严格加速度表，v4 以 `A0` 量纲一致地保留 `1×/5×/10×` 倍数。`A0±sigma_a` 是项目对“约等于”的操作化，`A0` 非有限/非正时 fail-closed。 |
