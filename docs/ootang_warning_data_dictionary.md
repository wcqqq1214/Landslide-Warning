...
- 为满足“先跑通藕塘”的实施需求，另有 [`ootang_operational_run.v1.draft.json`](../config/ootang_operational_run.v1.draft.json) 与 [`operational_run.py`](../code/warning/operational_run.py)。它以可执行范围表和数值字段表达 `V0±σ`、`5V0/10V0`、切线角、选段 `ΔV` MAD、两项佐证与两测点汇总；仅从 fit 期产生参数、在 calibration/test 执行。配置锁定基础协议 `1.3-draft` 的内容指纹与完整七项未决项，所有 CSV/manifest 输出 `operational_draft_not_formal`、`formal_warning_output=false`、`vajont_used=false`，并对可捕获的提升错误回滚旧快照。这不修改本字典的正式字段定义，也不将 KMeans 对照、严格 MVIF 失败或任何运行颜色升级为 Word 论文的正式方法或预警结论；详见 [`ootang_operational_run.md`](ootang_operational_run.md)。
