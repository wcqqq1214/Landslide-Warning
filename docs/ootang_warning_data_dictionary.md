...
- 用户已确认：MVIF 的有限 `t_f` 若在 fit 期多起点拟合中不可辨识，必须明确 `failed`，不能人为给 horizon 或由其生成正式 `V`、`σ`、`V0`、速度/切线角等级或预警。`mvif_fit_candidates.*` 只审计这条严格基线；当前藕塘 8 个 fit 记录均因 `tf_multistart_unstable` 失败，故它们不含任何下游阈值量。用户在审阅该结果后确认：这些失败不授权事后放宽多起点、Jacobian 秩或 `t_f` 一致性条件，也不将原始速度 KMeans 对照升级为指定 Word 路径。另经用户授权，`mvif_initial_slope_candidates.*` 是与该基线并存的**独立草案候选**：它不使用 `t_f` 一致性作为目标斜率的门禁，而是在拟合 MVIF 趋势上以王/安（2023）的 `5 d`、`0.99≤L≤1.01` 凹凸性判据选择最早连续匀速段，并对固定选段的 OLS 斜率做 95% selection-conditioned 剖面似然。原文的原始 `S–t`/最近段规则被明确改写为项目适配；局部位移增量形式使 `L` 不依赖截距 `C`，非逐日输入显式失败而不重采样。只有两侧剖面界有限时才写出 `candidate_v/candidate_sigma/candidate_v0`；这些字段仍为 `draft_candidate_not_formal`，不构成可靠 `t_f`、正式 `V0`、速度/切线角等级或预警，也不解除尚未冻结的稳定段规则。
...
| 逐点速度：`velocity` / `velocity_level` | `v_i=(U_i-U_{i-1})/(t_i-t_{i-1})`，单位 `mm/day`。`velocity_level` 是未来的五级单项等级，不等同于最终测点等级。 | 当前值使用相邻两次有效观测的实际 `Δt`；每个测点独立。Word 路径要求先从 MVIF 趋势项位移自动/人工确定初始稳定斜率。当前有两条隔离审计路线：fit 期有效原始速度 KMeans `V0` 仍只是对照；新增 MVIF 趋势候选只在严格逐日的 fit 位移上取最早连续 `L∈[0.99,1.01]` 的 5 日窗口串，并在固定段上输出有两侧剖面界的 `candidate_v/candidate_sigma/candidate_v0`。它未接入 `velocity_tangent_fit_calibration_diagnostics.csv`，避免与原始速度对照混淆。 | 首个速度为 `warmup`；缺失位移、无效日期或非正 `Δt` 产生明确无效状态，不插值。MVIF 候选的 fit 日期若非严格逐日，固定为失败而不擅自重采样。 | 指定论文式（5-3）为 `V0=max(1.5V,V+2σ)`，其中 `V` 是选定初始位移段的平均速率。表 5-4 的橙色列符号已由同章图 5-4（速率纵轴、`V0/5V0/10V0` 阈值线）核对为 `5V0≤V<10V0`；MVIF 最早段、趋势增量 `σ`、选段不确定性处理与 `V≈V0` 的 blue 容差仍未冻结：`stable_segment_selection`、`v0_blue_tolerance`。两个审计表都不输出速度等级或容差。 |
...
1. 每测点稳定段候选的正式接受（包括 MVIF 最早段的选段不确定性、趋势增量 `σ` 的约定）和速度五级的 blue/橙色边界；
