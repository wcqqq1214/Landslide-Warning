...
@docs/ootang_bplus_history_learning_plan.v1.17.md
@docs/ootang_bplus_history_learning_results.v1.17.md
@docs/ootang_bplus_history_diagnostics_plan.v1.18.md
@docs/ootang_bplus_history_diagnostics_results.v1.18.md
@docs/ootang_bplus_origin_learning_plan.v1.19.md
@docs/ootang_bplus_origin_learning_results.v1.19.md
@docs/ootang_bplus_origin_gradients_plan.v1.20.md
@docs/ootang_bplus_origin_gradients_results.v1.20.md
@docs/ootang_bplus_balanced_origin_plan.v1.21.md
@docs/ootang_bplus_balanced_origin_results.v1.21.md
@docs/ootang_bplus_input_transfer_plan.v1.22.md
@docs/ootang_bplus_input_transfer_results.v1.22.md
@docs/ootang_bplus_teacher_condition_review_plan.v1.23.md
...
- v1.25 连续预测四组有限学习已完成：3,600 更新、八组常尺度估计、主进程含首次核验 877.221 秒，exitcode=0，0 新物理积分/拟合/scaler 拟合。四组严格改善均 2/8，仅 432 日 MJ3/MJ1；IN_CARRY 两窗预测平均 RMSE 59.2967/25.4071，OOF_CARRY 59.4546/28.8947，均高于 P0 的 58.0122/22.2448 mm。CARRY 对 RESET 的预测两项同降 6/8、4/8，不能替代 P0 判据；四组两窗平均 CRPS 均未改善。所有拟合点窗及模型最终两块损失下降，远期历史局部梯度仍弱，不能视为已解决外推或已确定唯一根因。19 项测试及两次有限数值核验通过，各核对 180 checkpoint 身份、36 最终模型和独立 NumPy/概率评分；2,878 保护文件保持，425 产物已用 `601b2d5` 备份。**本版预算结束，整体目标未完成**。下一步先另版固定新连续模型的同权重状态开关及按步长修正诊断，再决定历史上下文表达；v1.18 是旧滑窗模型诊断，不能冒充新对照已完成。不重启此目录、不自动补训/调门/调权重或拼点，当前无下一版训练预算，原切分/旧负结果、日值 as-of 未知和条件历史边界保持。
...
- v1.17 起点历史学习对照已完成：P0/A/C/H，复用 342A/432B/612B，23 输入通道 ConvLSTM 的 C/H 仅差历史两通道屏蔽；1,800 更新、八组常尺度估计、215.982 秒，无新物理拟合/积分。H 相对 C 预测 RMSE/MAE 同时改善 7/8，但严格超过 P0 仅 3/8，预测平均 RMSE 58.9127/24.8298 仍高于 P0 的 58.0122/22.2448；C 严格 2/8、A 1/8。H 拟合改善、预测概率评分未改善，612 日 ATU1/ATU5 区间仍过宽且 MJ3 欠覆盖。13 项测试及 18 checkpoint 回放、全部样本/时序/概率核验通过，2,106 保护文件保持。**预算结束，整体目标未实现**。下一步先另版固定 H 权重的历史通道屏蔽和训练/外推修正需求诊断，再决定样本变化；屏蔽只是模型依赖诊断，不能作因果或另选模型。v1.5 已有失败 OOF 实验，重议时须说明区别。当前无下一版训练预算，不追加轮数或拼接种子，原始日值 as-of 未知、已知未来驱动和历史暴露边界保持。
- v1.19 起点历史 IN/OOF 有限学习已完成：共同拟合块及来源配对块各占权重 1/2，23 通道网络按当前前缀同步拟合；1,800 更新、六组常尺度估计、506.138 秒，0 新物理拟合/积分/scaler 拟合。预测平均 RMSE 为 IN 58.9612/25.1138、OOF 59.6529/33.6124，仍高于 P0 的 58.0122/22.2448 mm；严格改善 2/8、1/8。总损失均下降，但 612 日 OOF/seed2 拟合块损失增加 13.7187%，初始两块损失比 127.4170，不能据此直接断言梯度冲突或唯一根因。15 项测试、两次独立核验及 2,329 保护文件检查通过，172 产物含日志备份。**预算结束，整体目标未实现**。下一步另版固定 checkpoint 的两块梯度大小/方向诊断，再决定有限目标改法；本版无后续训练预算，不改变旧切分、教师、概率标准、旧 OOF/PINN 失败和条件历史回测边界。
- 开始或恢复本阶段工作时，先读下列导师目标、对应版本计划与结果，以及 `docs/progress.md` 最新本阶段记录，再实施、验证或报告；研究目标不等于已有结果。
- v1.18 固定历史网络依赖/修正需求诊断已完成：九个 H 权重，H/H0 同权重不同历史输入，科学求值 408 次、12.480 秒，封存复核另 282 次，0 更新/梯度/物理/尺度拟合。正式八点窗响应 RMS 0.2349–1.2551 mm，H 相对 H0 预测 RMSE/MAE 同降仅 3/8，不混同旧 H/C 的 7/8 或严格 B+ 判据。432 日 ATU1/ATU5 全窗反向；ATU5 预测需求 RMS 是训练 32.2119 倍，612 日 MJ3 为 11.2766 倍。612 日 ATU1/ATU5 未越训练需求范围仍失败，不把范围变化当唯一根因。13 项测试、13,512,960 输入指纹与 108,821 数值核验通过，2,292 保护文件保持，28 产物含日志已备份。**诊断结束，整体目标未实现**。下一步另版固定带历史/lead、当前前缀同步拟合的 IN/OOF 配对，先明确旧教师、日期权重、步长覆盖和有限预算；保留 v1.5 旧 OOF 失败并说明差异。H0 只作模型依赖探针，不选作新模型或改尺度；无下一版训练预算，原始日值 as-of 未知和条件历史边界保持。
- 旧报告、旧导师要求和旧待办仅为历史背景。阶段结束或替换时更新本入口，保留可追溯记录。

- v1.20 固定两目标梯度诊断已完成：90 checkpoint、270 梯度行、72 中心差分，科学进程 74.144 秒、2,592 前向/1,440 反向，封存复核另 1,728/960，0 更新/物理拟合/积分/scaler/尺度拟合。612 日 OOF 最终三种子的配对/拟合梯度比 13.0334/11.6620/17.0616，共同负梯度方向均使拟合块局部上升；e25/50/75/100 三种子均出现。432 日 OOF 最终两块可共同下降而预测仍失败，不作唯一根因或实际 Adam 轨迹判断。13 项测试、两次独立核验及 2,510 保护文件检查通过；原 NumPy matmul 警告保留，另以标量求和核验 2,160 项、最大差 2.842e-14，触发原因未定位。原归档 27 文件及两份标量核验文件已备份。**本版诊断结束，整体目标未实现**。下一步另版固定仅由训练内确定的两块尺度平衡规则、与 Adam/学习率的关系和有限配对预算；当前无后续训练预算，不扫描外层选权重、不把拟合恢复当预测成功，原始日值 as-of 未知、已知未来驱动和旧失败保持。

- v1.21 逐步梯度平衡有限对照已完成：1,800 更新、六组常尺度估计、552.014 秒，0 新物理拟合/积分/scaler 拟合。OOF 拟合 RMSE 降至 1.4150/1.6353 mm，预测为 59.1106/26.9027；IN 预测 59.0311/24.9176，均高于 P0 的 58.0122/22.2448，严格改善两组各 3/8。1,796/1,800 步两块实际同降，四步 paired 上升；NORM 最终 OOF paired 误差比冻结 EQ 更大，拟合恢复伴随取舍，概率目标仍失败。15 项测试、两次独立核验及 2,548 保护文件检查通过，176 个产物含日志已备份；90 checkpoint 梯度及日志核对不等于独立重训全部优化器轨迹。**本版预算结束，整体目标未实现**。按预定出口停止这一支继续调权重，下一步另版固定跨起点/时期的输入与修正需求检查；不据失败断言输入无信息，不自动追加训练或改旧切分/标准，条件历史回测与全部负结果保持。

- v1.22 同 lead 输入与修正迁移诊断已完成：唯一冻结输入 RMS 距离、一个四点共同邻居，1,800 候选边、1,260 过去参考距离，11.682 秒，0 新网络/梯度/训练/物理/回归/scaler/尺度拟合。探针正式两窗平均 RMSE：IN 57.5871/25.1131、OOF 57.3992/98.5816，P0 为 58.0122/22.2448 mm；预测两项同降两组各 2/8，不等于原拟合/预测四项严格判据。612 日 OOF 最近输入距离 180/180 日不高于过去候选间距离中位数，但四点都更差；选中 MJ3 修正同向 0/180，候选 432 来源同向 129/180 但幅度不等价，不改选邻居或断言数据无信息。14 项测试及两次独立核验通过，8,362,800 输入指纹值、204,608 数值、2,734 保护文件保持，31 产物含日志备份。**本版诊断结束，整体目标未实现**；按预定出口不扫描距离/邻居数，不晋升探针。下一步先审查输入/目标对物理教师条件与阶段变化的表达、有限教师数量及方法原文依据，再固定一项有限改法；没有下一版训练预算，原切分/模型/失败、原始日值 as-of 未知和条件历史回测边界保持。

- v1.23 教师条件/时序表达审查已完成：1,515 条原样本独立枚举、四组 54 维教师参数严格匹配，117,900 个窗口时间位置和 2,774 个保护文件核对通过，0 新网络/梯度/训练/物理/回归/scaler/尺度拟合。OOF paired 的 1/2/3 个旧教师各绑定一个外推起点，当前教师只进 anchor；不能把样本支持限制当成参数输入无效的证明。当前 23 通道已有隐式教师信息，两个时间窗口同位置相差 lead+1 日，各目标查询重置网络状态；是原设计而非新发现的 bug。原 PDF/ZIP 及 ConvLSTM、DeepAR、TFT 原文已核对，11 个审查产物已备份。**本版审查结束，整体目标未实现**。推荐下一版先登记并验证历史/未来日期对齐、编码状态传入逐日预测的有限接口，核对零读出恢复 B+、前缀一致、未来观测隔离和非零读出的历史梯度；学习另版固定预算。保留旧 M2/PINN 已有状态/递推及全部负结果，不把候选视为首次引入记忆或已验证有效，不同时增加教师参数/概率头/重调权重，原切分、日值 as-of 未知和条件历史回测边界保持。当前无新训练预算或导师接受记录。

- v1.24 历史编码/连续预测接口已实现并通过验证：同一 7,569 参数 ConvLSTM，日期对齐历史与未来零观测通道，hidden/cell 连续传递；0 更新、物理积分/拟合、scaler/概率尺度拟合。3,030 原查询及 15 真实案例的 30,815,400 输入/基线数值核对，零头恢复 B+、前缀/分段/未来物理尾部隔离均严格通过；NumPy 最大差 1.2212e−15 mm，120 方向导数最大差 2.0715e−11。主验证 6.557 秒，主验证/封存复核各 120 model 调用、285 序列段、13,920 cell 步、180 梯度、2,790 NumPy 步；294,120 数组值复现，2,787 保护文件保持，15 合成测试通过。未训练探针远期历史梯度很小，h612 第 180 日四点 1.2678e−35–3.1547e−34，不能把非零路径当长期记忆良好或断言训练必败。**接口验证结束，整体目标未完成**；下一版先登记有限学习预算、匹配对照、原逐点/概率评价及长期历史依赖诊断，再执行学习。本版未改遗忘门/通道/初始化；后续结构变化须另版登记，不能追改本版、放宽门限或冒称已训练。原切分/失败、日值 as-of 未知、当前 h 预处理和条件历史边界保持。81 主产物及核验旁档已分步备份，当前无新训练预算或导师接受记录。
