...
- 论文主线固定为：多源监测数据与逐点运动学特征 -> ConvLSTM 多测点概率位移预测 -> 预测区间及覆盖率评价 -> 四项预警指标 -> 自动生成的未来状态标签 -> NGBoost 五分类概率预警 -> SHAP 解释 -> 测点级与滑坡体级逐时预警 -> Vajont 外部案例验证。
...
- ConvLSTM 概率预测负责产生位移分位数/预测区间并评价覆盖程度；SHAP 用于识别模型依赖的候选主控因素。二者的作用必须在方法和结果中分别说明。
...
- 每张 SHAP 图和每段结论都必须明确被解释的模型、输出目标、背景样本和解释样本。若 SHAP 解释的是 NGBoost，只能称为“NGBoost SHAP”，不得写成“ConvLSTM+SHAP”。
- 论文若使用“ConvLSTM+SHAP”表述，SHAP 必须真实解释 ConvLSTM 的输入对位移预测或区间输出的贡献；否则应明确说明当前只完成了独立 NGBoost 的解释，并将 ConvLSTM 解释列为待完成或局限。
...
- Vajont 是导师要求的第二案例，使用 `data/vajont_fig5a_curves_2_3_4_5_58_mm_velocity.xlsx` 中的 `10d` 数据进行验证。
