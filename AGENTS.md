...
- **当前：C1—C10 已完成并独立核验，前三次路线复盘及 C10 补记已完成。** 先读简报、progress 与最新复盘。较好的 C8 在线机器学习候选开发/后期平均 RMSE 为 1.0781/1.7519 mm、覆盖 87.25%/80.40%，概率目标仍未过；C9 均值不变，覆盖增加但评分退步；C10 固定指数遗忘使均值与概率均退步，开发/后期平均 RMSE 1.1061/1.9144、覆盖 84.87%/79.64%，独立加权方程核验通过。停止 C7 经验分布、C9 尺度拆分及 C10 遗忘参数搜索。C11 已单独冻结（`docs/ootang_rolling_probability_candidate.c11.md`），准备实现：全四点保留 C8 DATA 均值，比较 B+ 与 DRIFT14 在宽概率分量中的作用，权重只从成熟 CRPS 学习；尚未运行，不把旧 DATA 均值收益归功新混合。完整目标尚未完成，不能把十轮或核验完成当作成功，也不把自定候选数当作用户授权结束。仍属原八小时，05:27:52 UTC 截止不变，新增候选需具体证据和独立事前方案；追加后期都是暴露后的探索性复核，旧 C4 唯一迁移失败保持。未 push，无关 Vajont Excel 保持。
