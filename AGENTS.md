...
- v1.4 的只读优化检查与后续方案已完成：检查有局部差分敏感/零敏感方向，单个内层窗口选模回放也失败。新方案分为同前缀 J1 续算诊断及固定 J0 的内部选模对照，总新预算上限 14,800 nfev；**当前仅确定方案，尚未实施或运行这些拟合**。后续执行依当前用户请求，不能把预算上限视为自动开跑或神经训练授权。
