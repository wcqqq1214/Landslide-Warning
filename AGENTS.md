...
- v1.12 有限学习方案已独立固定，**尚未实现训练器或运行**：S 直接通过同一 G/Day 训练和预测，复制 v1.9 各 e0 G 权重，3 前缀×3 种子×200=1,800 更新、1,800 秒硬上限；含 36 checkpoint 复算共 1,845 G/Day 前向，9 次原算法记录版回放，0 物理拟合/尺度网络。保留原优化器、倍率、切分、严格逐点标准和滚动概率尺度，不按外层选轮数或补训。具体计数、时序、物理/概率核验见新方案；该方案源于持续用户目标，不能视为导师已确认或效果已实现。
