...
- **当前有效状态：293 日复核已完成；用户随后要求继续，已形成 v2.3 水文蠕变概率 PINN 候选，尚未实现或训练。** 保留 B+，原 v2.1 和四参数力探针继续暂缓。第二次 6pro 补审已处理；按冻结 `e48d3e1` 对齐 1168 日参数及完整 293 日，M1/M2 均值、尺度均为 e0，没有神经均值增益；三组四点平均 RMSE 9.1242 mm、合并 RMSE 9.4506 mm。M0 概率指标不适用，不能搬用早期 P0。
