...
- **当前：C1—C15 已完成并适用独立核验，五次路线复盘完成，完整目标仍未达成。** C13/C14 误差头未超过原核心；C15 跨点 FULL 开发/后期平均 RMSE 1.7552/2.5172、CRPS 0.9775/1.4076 mm，26/27、24/27，两臂均差于原逐点版本。C15 原件/核验 `3e594cd`/`8a19c0c`，第五次复盘 `8031656`，51 项合成检查通过，核验不代替效果。停止误差单位/惩罚/滞后、固定跨点配置与旧概率层调整分支。下一步统一十五轮结果并补 C8 配对统计、非重叠和完整曲线；尚无冻结 C16，不以统计整理代替合格效果。原八小时 05:27:52 UTC 截止不变，所有后期追加为已暴露探索，C4 唯一迁移失败及原 293 日结论保持。未 push，无关 Vajont Excel 保持。
