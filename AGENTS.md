...
- **最新文档授权：用户提供6pro修改意见后要求先输出plan；已完成B+分组加性GP计划v2，仅为方案。** `docs/ootang_bplus_additive_gp_plan.v2.md`提出ADD与TIME_ONLY两个结构、各四点共8次拟合，检验物理条件分量相对纯时间的预测增量；完整观测方差包含后验交叉项。具体新配置、实际增益门槛和120分钟上限是文档提议，未获新训练启动授权，未实现或拟合，不继承GP v1余额。原GP均值小幅改善/平均概率改善与MJ3/MJ1概率失败的记录保持；新方案不能作为自动继续训练的入口。
