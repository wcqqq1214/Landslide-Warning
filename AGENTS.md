...
- **当前有效状态：按导师尾段补充要求，先定位范围并复核已有预测，暂缓 v2.1 PINN 实现与训练。** v2.2 是需求和实施顺序修订；尾段测点/日期及分段结果尚未确定。最新已完成实验 v2.0 按原标准未达标并已停止。方法仍只限定 ConvLSTM/PINN，保留 B+。
...
采用的方向解释：允许图示尾段预测偏差，优先主要预测时段的均值与概率质量；具体测点/日期仍待定位。
