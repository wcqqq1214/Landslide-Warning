...
- **当前：C1—C9 已完成并独立核验，第三次路线复盘已完成。** 先读简报、progress 和 `docs/ootang_rolling_route_review_2026-09-13.md` 第三次复盘。C8 开发/后期平均 RMSE 为 1.0781/1.7519 mm，覆盖 87.25%/80.40%；C9 均值不变，覆盖提高到 88.04%/82.58%，但 CRPS/区间评分退步，后期 25/27 仍未过。停止 C7 经验分布与 C9 尺度拆分分支；完整效果尚未完成，不把九轮或核验完成当作成功。C10 已另行冻结指数遗忘在线对照（`docs/ootang_rolling_probability_candidate.c10.md`，lambda=exp(-1/90)），当前准备实现，未运行；保留 C8 原等权对照、原初始化/正则/尺度，不搜索多个遗忘参数。仍属用户原八小时自主范围，05:27:52 UTC 截止不变；追加后期均为暴露后的探索性复核，旧 C4 唯一迁移失败保持，未 push。
