...
- **当前继续执行同一八小时自主目标：C7 经验误差分布校准准备中。** 新方案见 `docs/ootang_rolling_probability_extension.v3.6.md` 和配置 v3_6。先前六候选上限为助手资源安排，持续自主授权允许在原总截止内有依据地修正；累计上限改为 9、已完成六个不清零，第九个后再次复盘。原 v3.0 唯一后期 C4 的失败与全部冻结设计不变；C7 单独标为后期已暴露后的探索性复核，不重新宣称独立迁移成功，不修改 C5/C6 的禁止入口。只改变已兑现误差的分布校准，保留所有均值/raw_sigma，并与同历史池高斯对照配对。没有新八小时额度或新的逐命令批准要求，未达到完整效果，不标记目标完成。

- **本轮六候选、一次后期迁移和两次路线复盘已完成，完整效果目标未达成。** 先读 `docs/ootang_rolling_probability_brief_2026-09-13.md`、`docs/ootang_rolling_route_review_2026-09-13.md`、尺度对照与 progress。C4 正则回归属于机器学习；开发平均 RMSE 1.1105 mm、CRPS 0.5958、覆盖 86.74%，27/27 工作标准通过。后期平均 RMSE 1.7539（B_ANCHOR 5.5060、DRIFT1 4.0617）、CRPS 0.8205，但平均/ATU1/ATU5 覆盖不足，24/27 通过。C5 合并历史误差训练尺度失败；C6 只用最近完整折，开发 CRPS 0.5874、覆盖 86.89%，27/27 通过，但内部概率评分退步、未运行后期，不替换 C4。DATA 消融更好，物理额外贡献未证实；不称所有机器学习无效，也不称完整概率目标成功。21 项相关检查、全部模型/尺度/时间顺序与评分独立核验通过；C4 配对统计、非重叠统计和六张完整图已完成。六候选上限和唯一后期选择已用，本轮结束候选搜索，不新增 C7、不自动让 C5/C6 进入后期或调参后重报迁移成功；不是声称八小时耗尽。原长窗不改判，旧模型停止状态不变。
