...
- v1.8 已整理原 B+ 的降阶 PINN 方程接口，新增 `code/physics_guided_pinn/equations.py`，5 项合成代数/梯度检查通过，原 64 子步真实轨迹核验仍待完成。尚无状态网络、损失缩放、本构偏差选择、概率表示或 PINN 训练预算；下一步先做真实子步对齐，再另行固定有限训练方案。不能将经验限幅 L 或旧 M2 改名成已训练的经典 PINN。
