...
- v1.5 第一步已按“根据你的建议，一步步推进”完成误差结构与样本盘点，无新拟合或神经训练。误差方向跨窗变化，432/612 截止只有 180/360 个不同外推日期；事后去趋势不能算模型提升。下一步已单独固定同一 ConvLSTM 的 IN/OOF 样本来源对照：均值截止 342/432，随后 90/180 日估计常尺度；2 窗×2 策略×3 种子×100 轮，总 1,200 更新、20 分钟，无新物理拟合或 M2/PINN。方案刚固定，尚未实施/训练；须按其物理/均值/尺度隔离及固定停止规则执行，不自动加预算。整个研究目标仍在进行中。
