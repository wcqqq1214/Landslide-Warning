...
- **当前：C1—C17 已完成并适用独立核验，五次定期复盘及最后两轮收尾完成；完整效果目标仍未达成，停止新增 C18。** C16 用阶段开始前历史尺度和最新已兑现的一日误差学习长步长修正，PHYS 后期平均 RMSE 1.3931、CRPS 0.6931 mm，优于原 C8 DATA 的 1.7298/0.8135；覆盖 84.375% 仍未过，开发/后期 27/27、26/27。CORE 完整两段均值/CRPS 有增益，开发非重叠子集不支持稳定收益；B+ 对 CORE 的额外增益不稳定。C17 使用自身成熟误差做原因果校准，后期覆盖降到 82.39%，25/27，停止校准参数搜索。C16 原件/核验 `2e8c83f`/`64d86d5`，C17 `aaa9d39`/`4203bfc`；54 项相关合成检查通过。最终归并 `840b7e5`、日志备份补全 `3b981d2`，18 个运行 1,415 项原产物、29 条主记录、C16 的 112 项统计和六图完成；C8 的 98 项统计和六图也已完成，不再列为待办。原八小时截止 05:27:52 UTC 不变，当前仅收尾核对，不以原剩余时间恢复模型搜索。C4 唯一迁移失败、后来反复暴露及原 293 日结论保持。每步本地提交，未 push，无关 Vajont Excel 保持。科学效果与工作收尾、用户验收分列。
