...
- **当前继续同一八小时目标：C7 已完成并核验但覆盖未过，C8 在线均值学习准备中。** 见 C7 追加方案 v3.6 与 `docs/ootang_rolling_probability_candidate.c8.md`。累计上限 9、原截止 05:27:52 UTC 不变；第九个后复盘。C8 只在预测兑现后更新 Ridge 均值系数，固定原 C4 尺度/校准，保留静态 FULL/DATA 配对对照；两阶段计划 153,600 次监督系数更新。原 C4 唯一后期失败与旧冻结规则不改写，新增后期结果明确为已暴露数据上的探索性复核。C7 不再调分布，旧 C5/C6 guard 不改；不将局部评分、测试或文档完成称整体目标成功。
