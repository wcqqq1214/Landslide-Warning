...
- **本轮四候选与一次后期迁移已完成，完整效果目标未达成。** 先读 `docs/ootang_rolling_probability_brief_2026-09-13.md`、结果与 progress。C4 正则回归属于机器学习；开发平均 RMSE 1.1105 mm、CRPS 0.5958、覆盖 86.74%，27/27 工作标准通过。后期平均 RMSE 1.7539（B_ANCHOR 5.5060、DRIFT1 4.0617）、CRPS 0.8205，但平均/ATU1/ATU5 覆盖不足，24/27 通过。DATA 消融更好，物理额外贡献未证实；不称所有机器学习无效，也不称完整概率目标成功。全部模型/尺度/时间顺序与评分已独立核验，配对成块和非重叠统计、六张完整图已完成。三候选复盘与唯一后期选择锁保留，当前不据后期标签调参再称迁移成功；原长窗不改判，未自动恢复旧模型。
