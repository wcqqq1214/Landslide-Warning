...
2. 当前效果读[TiDE融合研究](docs/ootang_tide_fusion_results.v1.0.md)、[核验](docs/ootang_tide_fusion_validation.v1.0.md)、[回执](results/ootang_tide_fusion_v1/20260915/final_receipt.json)，实现追溯对应计划/配置/1272来源；再按需读[前轮K/H输入消融](docs/ootang_tide_features_results.v1.0.md)、[原TiDE配对](docs/ootang_tide_direct_results.v1.0.md)、[历史教师更新](docs/ootang_teacher_refresh_results.v1.0.md)、[训练预算](docs/ootang_training_sufficiency_results.v1.0.md)、[骨干×起点表达](docs/ootang_backbone_anchor_results.v1.0.md)及[GRU两因素](docs/ootang_gru_ablation_results.v1.0.md)，不恢复历史预算。
...
- 当前融合新增SPLIT/BOUND各123571参数，主支沿用KIN结构，H支独立编码后相加；两支从同初值共同训练，固定400更新。BOUND只改变H的起点化输出变换，cap由当前训练前缀30日位移变化RMS、下限1mm决定。H零输出回退本模型主支，不是独立旧KIN；限幅只约束附加H，总输出并不受界约束。新旧模型容量差异约12%，不作容量匹配单因素或物理因果结论。全部固定训练已完成，三项跨窗稳定假设均未获支持，不自动追加cap扫描、冻结主支、结构/轮数/λ/RL。下方K/H输入消融为历史完成版本。
