...
- **当前：C8 已完成并核验，C9 按步长尺度准备中。** 见 `docs/ootang_rolling_probability_candidate.c9.md`。C8 开发均值小幅改善、后期覆盖仍不足；原模型拟合段短/长步长原始覆盖失配已由保存参数重算。C9 只把跨步长共享的两尺度系数改为逐步长估计，480 次固定两参数优化，保留 C8 均值、旧对照及 RMS/反馈，不增加初值或边界。累计九候选安排、原 05:27:52 UTC 截止与第三次复盘不变。追加后期仅为已暴露数据上的探索性复核，旧 C4 唯一迁移失败不改判；测试或文档完成不等于整体效果通过。
