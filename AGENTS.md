...
- **最新用户请求：“根据这个计划，一步步进行”，已授权按 v2.4 计划实施。** JOINT 是唯一候选，DETACHED 是同条件梯度隔离对照；沿用 v2.0 两个完整 180 日窗口，不与最终 293 日或 v2.3 的 376 日混排。本轮总时限 2026-09-12 15:51:38—16:51:38 UTC，实现与检查截止 16:26:38 UTC，单次正式运行最多 600 秒；配置见 `config/ootang_convlstm_joint.v2_4.json`。当前准备实现，尚未训练；按计划完成后停止，不自动延长或重试。旧 PINN 停止状态和全部负结果保持。
...
- 开始或恢复工作先读本入口、最新 progress、方法决策、v2.3 结果和 293 日复核报告；按需追溯形成时方案。v2.3 已停止，不根据旧配置再次运行。v1.1–v1.25、v2.0 原结果/配置/源码及 v2.1/v2.2 记录保留，旧文档的“下一步”仅为历史背景。研究目标不等于已有结果或导师验收。
