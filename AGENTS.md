...
- **当前有效状态：v2.0 的一次有界实验已完成、未达标，停止该候选和自动扩展。** 用户明确只限定 ConvLSTM/PINN；非神经状态空间模型建议已撤回。该次请求的实施与核验已结束，不构成下一轮训练授权。
...
- 开始或恢复工作先读本入口、最新 progress、v2.0 结果和对应冻结方案。v1.1–v1.25 与 v2.0 的原结果、配置及源码均保留；旧文档中的“下一步”仅为历史背景。后续实验须按新的具体请求确定方案，仍限定 ConvLSTM/PINN；研究目标不等于已有结果或导师验收。
