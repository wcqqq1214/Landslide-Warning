...
- **当前：C1—C11 已完成并独立核验，完整概率目标仍未达成。** C11 使用 C8 DATA 全四点均值，PHYS 开发/后期覆盖 87.32%/81.34%，27/27、25/27，CRPS 差于核心；原件 `2c97930`、核验 `1dc25a1`，停止其 CRPS 权重组合。C12 已独立冻结（`docs/ootang_rolling_probability_candidate.c12.md`），准备实现：固定原均值和分量，仅以成熟 90% 区间评分学习 101 点网格权重，全部 CRPS 和覆盖门槛保持，无最低宽分量权重。第十二轮后作第四次路线复盘。已有 C7/C9/C10 停止状态及 C4 唯一迁移失败保持；所有后期追加为已暴露探索。原八小时 05:27:52 UTC 截止不变，不能把自定候选数或核验通过当作用户目标完成。未 push，无关 Vajont Excel 保持。
