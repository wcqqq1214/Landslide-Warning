...
- **最新用户请求：试一个 B+ 引导的直接多步概率 ConvLSTM，先输出 plan md。v2.4 当前仅有计划，未实现、未训练。** 计划以 JOINT 为唯一候选、DETACHED 为同条件梯度隔离对照，沿用 v2.0 两个完整 180 日窗口检验概率梯度回到均值分支的增量；不与最终 293 日或 v2.3 的 376 日混排。拟议后续 60 分钟总上限尚未起算，本轮不启动实现或训练。该请求是用户指定的新候选，不改写此前方法筛选结论；旧 PINN 停止状态和全部负结果保持。
