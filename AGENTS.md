...
- 最新研究完成（2026-09-15）：用户授权继续研究TiDE，完成物理特征K/H的2×2输入消融，分支`codex/tide-feature-ablation`，见`docs/ootang_tide_features_results.v1.0.md`及最终回执。同110392参数/180日历史/三种子/e400，新增KIN/HYD24拟合9600更新120检查点，精确复用DATA/PHYS24拟合120检查点；四起点完整293日发报、三主窗全部完成。KIN三窗RMSE22.907124/5.534082/7.540029，HYD29.660967/25.069860/8.476753mm，完整PHYS14.859012/12.114521/8.921779；最终B+9.124173。K存在时加入H第一窗降低RMSE8.048112，后两窗增加6.580439/1.381750；同种子KIN比PHYS同时改善1/2/3个、HYD0/0/1，两新组跨窗稳定判定均false。两新组最终MJ3均值仍差于B+，KIN三点覆盖不足80%；四组B+联合均0/3。751来源、240检查点/187837220独立数值、四图16面板/237192图形值、目视与11项解释检查完成，正式失败重试0、B+重拟合/物理前向0。独立14:49:06—15:49:06 UTC含准备核验，实际时间见回执，旧预算和余额不转用。下一假设为K/H分开编码与受限融合，尚未实现，不自动追加结构/轮数/λ/RL或按最终点成绩拼赢家。仅研究记录/CSV/PNG/SVG、本地commit，无push/PR/PDF汇报；未跟踪导师规则原样，用户/导师尚未验收。以下为历史完成版本。
...
2. 当前效果读[TiDE分组研究](docs/ootang_tide_features_results.v1.0.md)、[核验](docs/ootang_tide_features_validation.v1.0.md)、[回执](results/ootang_tide_features_v1/20260915/final_receipt.json)，实现追溯对应计划/配置/751来源；再按需读[原TiDE配对](docs/ootang_tide_direct_results.v1.0.md)、[历史教师更新](docs/ootang_teacher_refresh_results.v1.0.md)、[训练预算](docs/ootang_training_sufficiency_results.v1.0.md)、[骨干×起点表达](docs/ootang_backbone_anchor_results.v1.0.md)、[GRU两因素](docs/ootang_gru_ablation_results.v1.0.md)及[夜间简报](docs/ootang_overnight_graph_results.v1.0.md)，不恢复历史预算。
...
- 当前TiDE分组只改变标准化后K列5/6与H列7/8/9的输入可用性；历史/未来同时掩码，新组从同初值重训，旧DATA/PHYS精确复用。K为B+位移/日增量，H为含水状态和两类水头；共同降雨/水位等始终存在，K/H相关，H关闭不等于无水文信息。四组目标/单位/网络/样本/日期/教师/预算一致，不把指标交互解释为物理因果。下方原TiDE网络和信息边界保持，不按最终成绩选点/种子或恢复前轮预算。
