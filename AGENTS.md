...
- **最新状态：TCN 三阶段固定实验完成，训练停止，交付仍在收尾。** 见 `results/ootang_tcn_v1/20260914/`；运行结果已提交至 `2f6231d`。两臂内部共同选择400次，三个种子、三阶段共18次拟合/7200次更新。开发和后期七步长的平均 MAE/RMSE/CRPS/90%区间评分均优于 B_ANCHOR，但均落后于 DRIFT1 与 RR_DIRECT；两版 TCN 完整条件均未通过。开发均值/概率优胜者七步长均为 RR_DIRECT，共同推荐仅1—6天，第7天为空，不能混同排名与达标。后期7天 RMSE：TCN_DIRECT 0.856186、TCN_BRES 0.771781、DRIFT1 0.278587、RR_DIRECT 0.037318、B_ANCHOR 1.784162 mm；残差版90%覆盖76.8293%，七步长区间评分均比直接版更差，保留部分均值收益与概率限制。首次本地独立复算通过，核验器补充和回执尚未提交；最终 Markdown/汇总 CSV/图件尚未全部交付，不声明整体完成或导师验收。后续仅按原计划收尾，不追加训练、结构或候选，不制作PDF。
...
以下保留事前授权与历史阶段记录，不覆盖上述实际完成状态及本次维护范围。
