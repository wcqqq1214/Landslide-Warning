...
- **当前有效状态：导师 293 日任务的已有预测复核及交付已完成，保留 B+，原 v2.1 PINN 和四参数力探针继续暂缓。** 第二次 6pro 补审已由用户提供；已按冻结 `e48d3e1` 对齐原 1168 日参数与 2019-09-12—2020-06-30 全部 293 日。M1/M2 均值、尺度均为 e0，没有神经均值增益；三组四点平均 RMSE 9.1242 mm、合并 RMSE 9.4506 mm。M0 概率指标不适用，不能搬用早期 P0。
- 用户进一步明确：导师只是圈出尾段，表示局部预测不准可先不管，未指定日期。**不再把获取尾段日期作为当前报告或其他独立工作的前置条件。** 全窗及逐日误差保留；没有分段评分、尾段贡献或新数值验收门槛。若未来将分段用于选模/主要指标，须事前说明依据，不能按胜负截尾。新澄清不能让零修正变成神经增益。
...
- v2.1 方案/配置提交 `1ebb98a` 原样保留，尚未实现或训练；尾段容忍不放宽物理检查。旧 `Day.backward` 的梯度接口限制不是原 v2.1 张量残差对力断梯度的证据；施力历史与随机状态充分性仍属待验证问题。本次没有历史冲突反例或可辨识性验证。后续候选须先明确修正依据、历史状态表达、神经近似误差验收及效果标准，不直接恢复旧计划。
- v2.2 修订提交 `e78490d` 保留形成时的需求记录；原图现已定位为 PDF 第 13 页 ATU1/MJ3 的 293 日预测窗，与早期两窗不同。当前顺序和用户最新澄清以 293 日复核报告为准；不再把精确截点当成普遍前置条件。原 8/8 四项门槛不自动等同导师要求，旧负结果不改判。
- 不用“局部问题尚可继续诊断”自动延长路线。不得换版本重置预算、拼点/种子、事后选轮数或按胜负删日期来制造改善。方向修订及后续复核计入原 90 分钟剩余额度，扣减见 progress；无新训练预算，出现正证据也不自动开启下一轮。
- 开始或恢复工作先读本入口、最新 progress、293 日复核报告、v2.0 结果及 v2.2 修订。一次复核已经完成，报告完成即停，不自动追加探针、诊断版本或训练。v1.1–v1.25、v2.0 原结果/配置/源码及 v2.1 冻结方案均保留。旧文档中的“下一步”仅为历史背景，研究目标不等于已有结果或导师验收。
