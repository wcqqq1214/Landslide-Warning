...
- **当前有效状态：按导师尾段补充要求，先对齐预测时段并复核已有预测，暂缓 v2.1 PINN 实现与训练。** 原图已定位为 PDF 第 13 页 ATU1/MJ3，属于 2019–2020 年预测窗，与近期两个 2017–2018 年窗不同；精确尾段起始日期仍未确定。已整理一次 6pro 定向补审材料，尚未代用户发送或获得补审结论。方法仍只限定 ConvLSTM/PINN，保留 B+。
...
- v2.1 方案/配置提交 `1ebb98a` 原样保留，尚未实现或训练；其中的广义力偏差、末层贝叶斯近似和原力学门槛是暂缓候选的设计，不能因尾段允许偏差直接放宽物理检查。当前实施顺序以 v2.2 为准，不直接恢复旧计划。
- v2.2 修订提交 `e78490d`：不以图示尾段精确追踪为持续优化目标；先确定具体测点/日期，再对同范围的 B+/P0 和已有神经模型作一次主要时段、尾段及完整窗对照。原 8/8 四项门槛不再自动等同于导师要求，新具体效果判据待范围明确后另定；旧负结果不改判，范围不按模型成绩选择。
- 不用“局部问题尚可继续诊断”自动延长路线。不得换版本重置预算、拼点/种子、事后选轮数或按胜负删日期来制造改善。方向修订及后续复核计入原 90 分钟剩余额度，扣减见 progress；无新训练预算，出现正证据也不自动开启下一轮。
- 开始或恢复工作先读本入口、最新 progress、v2.0 结果及 v2.2 修订；后续依用户具体请求先定位和复核。v1.1–v1.25、v2.0 原结果/配置/源码及 v2.1 冻结方案均保留。旧文档中的“下一步”仅为历史背景，研究目标不等于已有结果或导师验收。
...
尚不能确定精确截点。来源及新补审问题见 [补审入口](docs/ootang_web_route_review_prompt.md)，不改写 v2.2 形成时记录。
完整窗口和困难尾段的误差继续保留，不能据此删数据或宣称旧模型已经达标。
来源、解释范围和实施顺序见 [v2.2 修订](docs/ootang_tail_scope_and_direction.v2.2.md)；原始导师文件不改写。
