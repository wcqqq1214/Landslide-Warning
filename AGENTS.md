...
- 最新研究完成（2026-09-16本地/2026-09-15 UTC）：用户授权回到原TiDE_KIN，分支`codex/tide-kin-horizon-diagnostic`，见[诊断结果](docs/ootang_tide_kin_diagnostic_results.v1.0.md)、[核验](docs/ootang_tide_kin_diagnostic_validation.v1.0.md)、[回执](results/ootang_tide_kin_diagnostic_v1/20260915/final_receipt.json)。60原检查点/1816起点—前缀组合/27240训练起点评价、48条可见性种子路径完成；新拟合/更新/B+拟合/物理前向0，原四条发报及e400不变。第一窗前30日KIN/DRIFT1平均RMSE7.987134/0.651233mm；三主窗尾段期望损失权重14.259250%/22.361684%/26.677111%。最终MJ3尾段SSE份额97.991179%、90%覆盖15.044248%，种子离散占平均MSE1.348528%；原KIN最终7.540029、B+9.124173mm/联合0/3不变。283来源、45766892数值、四图16面板/21088图形值、目视及11项解释核验通过，正式失败重试0；612原无概率尺度保持缺失。独立17:27:16—18:27:16 UTC含全部准备核验，实际结束见回执，旧预算和余额不转用。结论为早期偏差/远期监督权重/未来输入依赖多问题并存，尚未识别唯一原因；下一候选是按h隔离更晚协变量的KIN规则，尚未实施或训练，不同时改损失、不追加H/检查点选择/结构扫描/轮数/λ/RL。只本地Markdown/CSV/PNG/SVG，无push/PR/PDF汇报；导师规则原样，用户/导师尚未验收。下方均为历史完成版本。

- 历史研究完成（2026-09-16本地，2026-09-15 UTC）：用户授权冻结原KIN后进行历史样本外残差校正配对，分支`codex/tide-frozen-correction`，见`docs/ootang_tide_correction_results.v1.0.md`及最终回执。CAL/HCAL同13179可训练参数/三种子/e400，只有H可读性不同，原110392参数KIN精确保留。39新拟合15600更新、201新/启动+60旧=261检查点、396条历史发报×3种子、四外层完整293日和三主窗完成。完整可用路径0/20/200/396，基模型前缀0/1/5/9，612预设零校正；不把重叠路径称独立重复。CAL三窗RMSE42.602315/17.984216/14.133238，HCAL42.799103/19.006908/10.541766，KIN22.907124/5.534082/7.540029mm；两新版集成MAE/RMSE/CRPS三窗均退步。HCAL对CAL同种子同时改善3/0/0，与集成方向反转保留，两稳定判定均false；三组B+联合均0/3。CAL最终概率门过B+但均值失败，HCAL最终MJ3覆盖62.12%。2216来源、89997623数值、四图16面板/257388图形值及目视/11项解释核验完成，正式失败重试0、B+重拟合/前向0。核验v1通过后只删未用导入并复跑v2，原源码留档。新独立16:41:32—18:11:32 UTC包含准备核验，实际结束见回执，旧预算不恢复。按冻结计划收束当前H附加校正路线，不追加cap/结构/轮数/λ/RL或拼点赢家；残差迁移失配尚未确认为唯一原因。下一问题可回到原KIN跨时段误差/长距离监督，尚未实施。仅研究记录/CSV/PNG/SVG、本地commit，无push/PR/PDF汇报，导师规则原样，用户/导师尚未验收。以下为历史完成版本。

- 历史研究完成（2026-09-16本地，2026-09-15 UTC）：TiDE分开编码与受限H融合。 [研究记录](docs/ootang_tide_fusion_results.v1.0.md)、[四张研究图](figures/ootang_tide_fusion_v1/20260915/README.md)、[回执](results/ootang_tide_fusion_v1/20260915/final_receipt.json)。用户授权“继续验证假设”，分支`codex/tide-hydro-fusion`。SPLIT/BOUND同123571参数、180日历史、三种子/e400，从头联合训练；BOUND以训练段30日位移变化RMS限制独立H支，非事后裁剪或物理约束。24新拟合9600更新120检查点，精确复用KIN/PHYS24拟合120检查点，四起点完整293日发报/三主窗完成，失败重试0、B+重拟合/前向0。SPLIT三窗RMSE52.630851/15.600443/9.230440，BOUND25.340337/7.703676/9.038071mm，最终B+9.124173。BOUND相对SPLIT平均MAE/RMSE/CRPS三窗改善，但完整均值门只过第一窗、同种子同时改善3/2/1个；两新组平均MAE/RMSE三窗均差于旧KIN，BOUND对SPLIT/KIN/PHYS稳定判定均false。最终BOUND对B+平均RMSE仅改善0.943667%，MJ3/MJ1逐点失败，90%覆盖41.64%/56.31%；四组B+联合均0/3。1272来源、240检查点/188863268数值、四图16面板/247192图形值、目视及11项解释检查完成。图v2仅更正一处“实线→曲线”，v1和源代码留档，其余三PNG逐字节一致；封装时间函数调用异常在写入前发生并已更正，无科学数据改动。冻结1b29db03、实现c7836189、实验3d746aee、核验ee100daa，图件和最终记录各本地提交。独立15:51:35—16:51:35 UTC含准备核验，实际结束见回执；旧预算/余额不转用，不追加cap/训练/结构/λ/RL，不按已见最终点成绩拼赢家。未来可区分主支联合更新与H额外信息，尚无新实验。仅研究记录/CSV/PNG/SVG，无push/PR/PDF汇报；未跟踪导师规则原样。实施核验完成、性能目标未达、用户/导师尚未验收。

- 历史研究完成（2026-09-15）：用户授权继续研究TiDE，完成物理特征K/H的2×2输入消融，分支`codex/tide-feature-ablation`，见`docs/ootang_tide_features_results.v1.0.md`及最终回执。同110392参数/180日历史/三种子/e400，新增KIN/HYD24拟合9600更新120检查点，精确复用DATA/PHYS24拟合120检查点；四起点完整293日发报、三主窗全部完成。KIN三窗RMSE22.907124/5.534082/7.540029，HYD29.660967/25.069860/8.476753mm，完整PHYS14.859012/12.114521/8.921779；最终B+9.124173。K存在时加入H第一窗降低RMSE8.048112，后两窗增加6.580439/1.381750；同种子KIN比PHYS同时改善1/2/3个、HYD0/0/1，两新组跨窗稳定判定均false。两新组最终MJ3均值仍差于B+，KIN三点覆盖不足80%；四组B+联合均0/3。751来源、240检查点/187837220独立数值、四图16面板/237192图形值、目视与11项解释检查完成，正式失败重试0、B+重拟合/物理前向0。独立14:49:06—15:49:06 UTC含准备核验，实际时间见回执，旧预算和余额不转用。下一假设为K/H分开编码与受限融合，尚未实现，不自动追加结构/轮数/λ/RL或按最终点成绩拼赢家。仅研究记录/CSV/PNG/SVG、本地commit，无push/PR/PDF汇报；未跟踪导师规则原样，用户/导师尚未验收。以下为历史完成版本。

- 上一轮研究完成（2026-09-15）：用户要求继续寻找模型、无需导师汇报，授权TiDE直接预测配对，分支`codex/tide-direct-physics`。见`docs/ootang_tide_direct_results.v1.0.md`和最终回执。同110392参数/180日历史/三种子/e400，24拟合9600更新、120检查点与四条293日路径完成；B+重拟合/物理前向0。DATA三主窗RMSE50.874904/33.672205/7.278984，PHYS14.859012/12.114521/8.921779mm，最终B+9.124173。PHYS平均MAE/RMSE/CRPS三窗低于G_CACHED，但同种子均值同时改善仅1/2/2；物理配对同向种子3/3/0，稳定均值false。两版B+联合均0/3；DATA最终ATU5退步/MJ3覆盖67.58%，PHYS最终MJ3/MJ1均值保护失败而概率门对B+通过。261来源、93991564独立数值、三图12面板/227476图形值与目视通过。原SciPy兼容故障及同版失败留档，独立Python3.12.13/SciPy1.16.3运行，原环境不改；预检NumPy告警以非BLAS前向复核，正式失败重试0。独立14:13:11—16:13:11 UTC含准备核验，实际完成见回执，旧预算/余额不转用。下一研究优先物理特征分组，本轮不自动追加消融/结构/轮数/RL，不按最终点成绩拼接赢家。仅研究记录/CSV/PNG/SVG、本地commit，无push/PR/PDF汇报；导师规则未跟踪文件保持，用户/导师尚未验收。以下记录是已完成的历史实验。

- 历史完成（2026-09-15）：用户授权的历史B+教师更新政策配对，分支`codex/teacher-refresh-pairing`，见`docs/ootang_teacher_refresh_results.v1.0.md`及最终回执。两臂同GRU起点表达/共同60日网格/三种子/e200，10次历史物理更新、24神经拟合4800更新、96检查点及四条293日发报全部完成。G_CACHED三个主要窗RMSE19.372056/22.660456/10.600134，G_REFRESH24.684374/23.804193/11.068245mm；更新组集成MAE/RMSE三窗均退步，同种子同时改善0/2/1个；两组B+联合门均0/3，最终B+9.124173。历史拟合8/10降低RMSE，但全部物理拟合达到800次上限、未收敛，不能断言所有更新教师方法无效。203来源、5588338独立数值、三图12面板/222772图形值核验与目视通过；18项原运行时依赖已按原字节归档，原资料不改。新独立11:01:47—13:01:47 UTC自限窗口含准备核验，实际用时见回执，旧预算及余额不转用。按冻结计划收束本轮路线，不追加教师密度/轮数/结构/λ/RL，只本地commit/Markdown/CSV/PNG/SVG，不push/PDF。图件封装相对路径错误仅修目录根，图形/预测保持；用户未跟踪导师制图规则原样，用户/导师尚未验收。
...
2. 当前诊断读[原KIN距离研究](docs/ootang_tide_kin_diagnostic_results.v1.0.md)、[核验](docs/ootang_tide_kin_diagnostic_validation.v1.0.md)、[回执](results/ootang_tide_kin_diagnostic_v1/20260915/final_receipt.json)，实现追溯计划/配置/283来源；再按需读[冻结KIN校正](docs/ootang_tide_correction_results.v1.0.md)、[联合融合](docs/ootang_tide_fusion_results.v1.0.md)、[K/H输入消融](docs/ootang_tide_features_results.v1.0.md)、[原TiDE配对](docs/ootang_tide_direct_results.v1.0.md)等历史版本，不恢复历史预算。
...
- 当前KIN诊断只重放原60检查点/训练日程，无梯度更新、重选或概率校准；训练前缀内标准化/成熟目标不冒充历史独立预测。期望距离系数mean(1[h≤L]/L)与实际抽样系数不是梯度。完整发报、三种子、四点、固定距离及612无sigma均保持。可见性d30/90/180/293只改未来驱动/K与known掩码，相对完整输入在共同h1…d计算变化，不能当实测RMSE、泄露证据、移除晚期信息有益或物理因果。不同d的平均日期集合不同；模型完整293日条件驱动原本合法。下一条按h隔离更晚协变量的规则只是未实施假设，若改接口/参数量必须新版本披露；不与距离损失再加权同时变更。旧全部效果门与负结果保留，下方为历史边界。

- 前轮冻结KIN校正：原基模型完全不进入头优化器；两头同13179参数/样本/种子/单位/cap，只改变H三列读取。CAL头仅距离/known/点ID，完整模型仍有K与共同水文驱动。历史基模型r≤发报m、目标始于m；只有m+293≤当前n的整条条件驱动和标签才入校正训练，禁止用当前KIN训练段残差替代。历史预测为当时可用数据重建，不冒充过去实际发布。612无完整路径，预设零头/无优化；早期480基模型最长直接监督48天，其残差不等同最终1168模型。全部261检查点和发报已核验，旧KIN精确保留；限幅有效不等于纠偏方向正确。新旧冻结/联合训练同时改变残差来源与训练流程，不作冻结单因素或因果结论。按停止规则关闭当前H附加校正路线，下一研究须独立授权/冻结，下方联合融合和特征消融均为历史完成版本。

- 前轮融合新增SPLIT/BOUND各123571参数，主支沿用KIN结构，H支独立编码后相加；两支从同初值共同训练，固定400更新。BOUND只改变H的起点化输出变换，cap由当前训练前缀30日位移变化RMS、下限1mm决定。H零输出回退本模型主支，不是独立旧KIN；限幅只约束附加H，总输出并不受界约束。新旧模型容量差异约12%，不作容量匹配单因素或物理因果结论。全部固定训练已完成，三项跨窗稳定假设均未获支持，不自动追加cap扫描、冻结主支、结构/轮数/λ/RL。下方K/H输入消融为历史完成版本。

- 前轮TiDE分组只改变标准化后K列5/6与H列7/8/9的输入可用性；历史/未来同时掩码，新组从同初值重训，旧DATA/PHYS精确复用。K为B+位移/日增量，H为含水状态和两类水头；共同降雨/水位等始终存在，K/H相关，H关闭不等于无水文信息。四组目标/单位/网络/样本/日期/教师/预算一致，不把指标交互解释为物理因果。下方原TiDE网络和信息边界保持，不按最终成绩选点/种子或恢复前轮预算。
