...
@docs/progress.md
@docs/ootang_convlstm_direct_plan.v2.0.md

历史规格与结果按 `docs/README.md` 查证；不把其中形成时的“当前计划”自动加载为当前任务。

- **当前有效状态：用户在纠偏与清理后要求继续开始，并明确只限定 ConvLSTM/PINN。** 本次仅执行 v2.0 一个直接多步 ConvLSTM 候选；不转入非神经状态空间模型，不恢复自动扩展实验与诊断。当前请求授权这一次有界实施，旧持续目标不构成后续预算。
...
- 本次仅按 v2.0 冻结方案实施、核验和评估，不运行旧诊断或重训旧变体；不重拟合 B+。训练和必要核验均计入本次总投入。
- v2.0 方案与配置先提交再训练。全流程固定 120 分钟，2026-09-11 07:33:17–09:33:17 UTC，含准备、实现、检查、计算、解释与提交；一次运行另限 1800 秒。最终四项均值要求 8/8，概率主评分和逐点覆盖/宽度判据见冻结方案。
